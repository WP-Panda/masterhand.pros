    ...
    <div class="fre-page-section">
        <div class="profile-endorsements">
            ...
            <?php
                ...
                $wpdb->prepare("SELECT ID, post_title FROM $wpdb->posts WHERE post_author = %s", $user_ID )
            );
                $professional_search = $wpdb->get_results(
                $wpdb->prepare("SELECT post_id FROM $wpdb->postmeta WHERE meta_key = 'professional_id' AND meta_value = %s", $user_ID )
            );
                ...
                foreach($professional_search as $post){
                        ...
                        $wpdb->prepare("SELECT post_author, post_title FROM $wpdb->posts WHERE ID = %s", $post->post_id ));
                ...
                <div class="fre-profile-box page-referrals_list">
                    ...
                    <?php $query = new WP_Query(array('post_type'=>'fre_profile', 'author' => $prof_ids, 'posts_per_page' => 1, 'orderby'=> 'date', 'order'=>'desc'));?>
                     ...
                     <?php if (  $query->max_num_pages > 1 ) { ?>
                        ...
                        <a id="true_loadmore" class="fre-submit-btn endors_loadmore"><?php echo __('Show more',ET_DOMAIN);?></a>
                     <?php } ?>
                </div>
                <?php wp_reset_query();
                } else {?>
                    <?php _e('No SafePay Deals yet', ET_DOMAIN) ?>
                <?php } ?>
                   ...
                   <div class="page-referrals_item" data-id="<?=$metas?>">
                        <div class="row">
                            <div class="col-sm-9 col-xs-7">
                                <a class="hidden-xs" href="<?=get_author_posts_url($metas)?>"><?php echo get_avatar($metas, 70); ?></a>
                                <a class="name"  href="<?=get_author_posts_url($metas)?>"><?php echo get_the_author_meta('display_name', $metas)?></a>
