...
if(!$user_email || !$user_answer ){ // if user email or radio button is empty
  header('Content-type: application/json');
      $response = ['response'=> 'error', 'error_type'=>'user_error', 'text' => 'email or answer is empty or not correct'];
 ...
 if(filter_var($user_email, FILTER_VALIDATE_EMAIL)){ //validate email
   ...
   header('Content-type: application/json');
      $response = ['response'=> 'error', 'error_type'=>'email', 'text' => 'email is empty or not correct'];
      echo json_encode($response);
   die;
 }
}
