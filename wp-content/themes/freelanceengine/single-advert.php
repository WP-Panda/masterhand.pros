	...
	<div class="fre-post-project-box">
		<form id="edit-advert" class="" role="form" method="POST" onsubmit="return false;">
			<div class="fre-input-field">
				<label class="fre-field-title" for="fre-project-title"><?php _e('Advert title', ET_DOMAIN); ?></label>
				<input type="hidden" name="post_id" value="<? echo $post->ID ?>">
				<input class="input-item text-field" id="fre-project-title" type="text" name="post_title" maxlength="100"
					   value="<? echo $post->post_title ?>">
			</div>
			<div class="fre-input-field">
				<label class="fre-field-title" for="fre-project-describe"><?php _e('Description', ET_DOMAIN); ?></label>
				...
				<ul id="listImgPreviews" class="portfolio-thumbs-list row image">
					...
					if(!empty($attachments)) {
						foreach ($attachments as $attachment) { ?>
							<li class="col-sm-3 col-xs-12 item" data-id="<?=$attachment->ID;?>">
                        ...
                        <select name="country" id="country"
                            ...
                            <?php if ($query_country->num_rows > 0) {
                                while ($row = $query_country->fetch_assoc()) {
                                    if (!empty($country) && $country == $row['id']) $flag = 'selected';
                                    else $flag = '';
...
?>
	<div class="fre-page-title">
		<h2><?php the_title(); ?></h2>
		<? if ($user_ID == $post->post_author && $post->post_status == 'publish') { ?><a href="?post_edit"><? _e('Edit') ?></a> <? }
		?>
	</div>
