    ...
    <div class="profile-freelance-portfolio">
        <div class="row">
            ...
            <div class="<?php echo $is_edit ? 'col-sm-6' : '' ?> col-xs-12">
                <div class="freelance-portfolio-title"><?php _e( 'My best works:', ET_DOMAIN ) ?></div>
            </div>
			<?php
			if ( $is_edit ) {
				if ( ! $flag || count( query_posts( $query_args ) ) < $count_portfolio ) {
                    ...
                    <div class="col-sm-6 col-xs-12">
                        <div class="freelance-portfolio-add"><p style="color:#1fbdbd;font-weight: 600;">You can
                                add only <?= $count_portfolio ?> works</p></div>
                    </div>
					<?php
				}
			}
