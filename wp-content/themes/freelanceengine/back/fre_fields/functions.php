<?php
	...
	function ae_order_field_by_menu_order( $orderby, $query ) {
		global $wpdb;
		if ( $query->query_vars[ 'post_type' ] != 'ae_field' ) {
			return $orderby;
		}
	...
	function et_render_custom_meta( $post ) {
		...
		$custom_fields = $field_object->fetch( 'ae_field', [
			'meta_query' => [
				[
					'key'     => 'field_for',
					'value'   => $post_type,
		...
		if ( ! empty( $custom_fields ) ) {
			foreach ( $custom_fields as $key => $field ) {
				$tax_list    = '';
				$field_value = '';
				if ( $field->field_for != $post_type || $field->type == 'tax' ) {
					continue;
				}
	...
	if ( ! function_exists( 'ae_add_field_to_submit_form' ) ) {
		...
		function ae_add_field_to_submit_form( $post_type, $post ) {
			global $ae_post_factory;
			$field_object = $ae_post_factory->get( 'ae_field' );
			$fields       = $field_object->fetch( 'ae_field', [
				'meta_query' => [
					[
						'key'     => 'field_for',
						'value'   => $post_type,
	...
	function ae_field_script() {
		...
		wp_enqueue_script( 'ae_field', plugin_dir_url( __FILE__ ) . '/assets/field.js', [
			'underscore',
			'backbone',
			'appengine'
		], '1.0', true );
		...
		//wpp_dump($date_format);
		wp_localize_script( 'ae_field', "date_format", [ 'date_format' => $date_format ] );
	}