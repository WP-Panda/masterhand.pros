...
//*********************** END NOT USED ******************
    ...
    $ae_post_factory->set(PROFILE, new AE_Posts(PROFILE, array('project_category'), array(
        'et_professional_title',
        'hour_rate',
        'et_experience',
        'et_receive_mail',
...
function fre_register_portfolio()
{
    ...
    global $ae_post_factory;
    $ae_post_factory->set(PORTFOLIO, new AE_Posts(PORTFOLIO, array('project_category'), array('portfolio_link')));
        ...
        if (isset($_REQUEST['query']['s']) && $_REQUEST['query']['s'] != '' && $query->query_vars['post_type'] == 'fre_profile') {
            ...
            foreach ($q['search_terms'] as $term) {
                $terms_query = get_terms( array(
                    'taxonomy'      => array( 'project_category' ),
                    'hide_empty'    => 0,
                    'search'        => $term,
                ) );
...
//            $fields .= ' , wp_activity_rating.rating+wp_activity_rating.pro_rating as rating ';
    ...
    function filter_where_profile($where, $query)
    {
        ...
        if (!empty($query->query['with_companies'])) {
            ...
            $table_meta = array('wp_postmeta', 'mt1', 'mt2');
                ...
                if ($request['user_email']) {
                    ...
                    if ($user_ID == $request['post_author'] && $request['user_email'] != $current_user->user_email && $request['user_email'] != $current_user->user_new_email) {
                        ...
                        if ($result != false && !is_wp_error($result)) {
                            fre_update_user_new_email(array('user_email' => $current_user->user_email, 'ID' => $request['post_author']));
                        }
                    }
                }
...
//*********************** END NOT USED ***************************
        ...
        $text_option = __('Select an option', ET_DOMAIN);
        $html_edit_select_profile_category = '<select class="fre-chosen-multi" name="project_category"'
            . ' multiple data-first_click="true" data-placeholder="' . $text_option . '">';
        $profile_id = get_user_meta($result->post_author, 'user_profile_id', true);
        if ($profile_id) {
...
//                $AE_PostAction->__construct(PORTFOLIO, array('skill'));
                $AE_PostAction->__construct(PORTFOLIO, array('project_category'));
