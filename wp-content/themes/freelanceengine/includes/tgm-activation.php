		...
		class TGM_Plugin_Activation{
			...
			public function notices() {
				...
				if ( ! empty( $message ) ) {
					...
					$action_links = apply_filters( 'tgmpa_notice_action_links', [
		...
		class TGMPA_List_Table extends WP_List_Table{
			...
			public function column_plugin( $item ) {
				...
				if ( ! isset( $installed_plugins[ $item[ 'file_path' ] ] ) ) {
					$actions = [
						'install' => sprintf( '<a href="%1$s" title="' . __( 'Install', 'tgmpa' ) . ' %2$s">' . __( 'Install', 'tgmpa' ) . '</a>', wp_nonce_url( add_query_arg( [
								'page'          => TGM_Plugin_Activation::$instance->menu,
								'plugin'        => $item[ 'slug' ],
								'plugin_name'   => $item[ 'sanitized_plugin' ],
								'plugin_source' => $item[ 'url' ],
								'tgmpa-install' => 'install-plugin',
							], admin_url( 'themes.php' ) ), 'tgmpa-install' ), $item[ 'sanitized_plugin' ] ),
					];
				} // We need to display the 'Activate' hover link.
                elseif ( is_plugin_inactive( $item[ 'file_path' ] ) ) {
					$actions = [
						'activate' => sprintf( '<a href="%1$s" title="' . __( 'Activate', 'tgmpa' ) . ' %2$s">' . __( 'Activate', 'tgmpa' ) . '</a>', add_query_arg( [
								'page'                 => TGM_Plugin_Activation::$instance->menu,
								'plugin'               => $item[ 'slug' ],
								'plugin_name'          => $item[ 'sanitized_plugin' ],
								'plugin_source'        => $item[ 'url' ],
							...
							], admin_url( 'themes.php' ) ), $item[ 'sanitized_plugin' ] ),
					];
				}
			...
			public function process_bulk_actions() {
				...
				if ( 'tgmpa-bulk-install' === $this->current_action() ) {
					...
					], admin_url( 'themes.php' ) ), 'bulk-plugins' );
					$method = ''; // Leave blank so WP_Filesystem can populate it as necessary.
