<?php
	...
	class Fre_Profile_Category extends AE_Base{
		function __construct() {
			$this->add_action( 'profile_category_add_form_fields', 'fre_add_form_fields' );
			$this->add_action( 'created_profile_category', 'fre_save_tax_meta', 10, 2 );

			$this->add_action( 'profile_category_edit_form_fields', 'fre_edit_tax_group_field', 10, 2 );
			$this->add_action( 'edited_profile_category', 'fre_update_tax_meta', 10, 2 );
		...
		function fre_add_form_fields( $taxonomy ) {
			...
			// Get the meta value
			$value  = get_term_meta( $term_id, 'profile_category_image', true );
			$hidden = empty( $value ) ? ' style="display: none;"' : ''; ?>
            <div class="form-field term-group">
                <label><?php _e( 'Taxonomy image', ET_DOMAIN ) ?></label>
                <div>
                    <img id="ae-tax-images-photo"
                         src="<?php echo esc_url( wp_get_attachment_image_url( $value, 'full' ) ); ?>"<?php echo $hidden; ?> />
                    <input type="hidden" name="<?php echo $taxonomy; ?>_image" id="<?php echo $taxonomy; ?>_image"
                ...
                <a href="<?php echo esc_url( $remove_url ); ?>"
                   class="button ae-tax-images-remove"<?php echo $hidden; ?>>
                ...
                <div class="featured-tax">
                    ...
                    <label for="featured-tax" class="left"
                           style="display: inline;"><?php _e( 'Featured taxonomy', ET_DOMAIN ); ?></label>
                </div>
            </div>
            <div class="clearfix"></div>
			<?php
		...
		public function fre_save_tax_meta( $term_id, $tt_id ) {
			...
			if ( isset( $_POST[ 'profile_category_image' ] ) && '' !== $_POST[ 'profile_category_image' ] ) {
				$group = sanitize_title( $_POST[ 'profile_category_image' ] );
				update_term_meta( $term_id, 'profile_category_image', $group );
			}
		}
		...
		function fre_edit_tax_group_field( $term, $taxonomy ) {
			...
			$value      = get_term_meta( $term->term_id, 'profile_category_image', true );
			$hidden     = empty( $value ) ? ' style="display: none;"' : ''; ?>
            ...
            <tr>
                ...
                <td>
                    <div>
                        <img id="ae-tax-images-photo"
                             src="<?php echo esc_url( wp_get_attachment_image_url( $value, 'thumbnail' ) ); ?>"<?php echo $hidden; ?> />
                        <input type="hidden" name="<?php echo $taxonomy; ?>_image" id="<?php echo $taxonomy; ?>_image"
                               value="<?php echo esc_attr( $value ); ?>"/>
                    </div>
                    <a class="button-secondary ae-tax-images-media">
		...
		public function fre_update_tax_meta( $term_id, $tt_id ) {
			...
			if ( isset( $_POST[ 'profile_category_image' ] ) && '' !== $_POST[ 'profile_category_image' ] ) {
				...
				update_term_meta( $term_id, 'profile_category_image', false );
			}
		}

		public function fre_tax_enqueue_scripts() {
			wp_enqueue_media();
			wp_enqueue_script( 'fre-tax-images', get_template_directory_uri() . '/assets/js/fre_taxonomy_custom.js', [
				'jquery',
				'underscore',
				'backbone',
				'appengine'
			...
			// Localize
			wp_localize_script( 'fre-tax-images', 'i10n_WPTermImages', [
