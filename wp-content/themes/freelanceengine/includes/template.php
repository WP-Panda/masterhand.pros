<?php
		...
		function fre_profile_button() {
			...
			if ( fre_share_role() || ae_user_role() == FREELANCER ) {
                ...
                <a href="<?php echo et_get_page_link( [ 'page_type'  => 'profile',
				                                        'post_title' => __( "Profile", ET_DOMAIN )
	...
	if ( ! function_exists( 'fre_project_button' ) ) {
		...
		function fre_project_button( $echo = false ) {
			...
			if ( ae_user_role() != FREELANCER ) {
				?>
                <a href="<?php echo et_get_page_link( [ 'page_type'  => 'submit-project',
			...
			// current user is an employer
			?>
            <a href="<?php echo get_post_type_archive_link( PROJECT ) ?>" class="btn btn-sumary btn-post-project"><i
	...
	function ae_edit_post_button( $post ) {
		if ( $post->post_status == 'pending' ) { ?>
            <a title="<?php _e( "Edit", ET_DOMAIN ); ?>" target="_Blank"
               href="<?php echo et_get_page_link( 'edit-project', [ 'id' => $post->ID ] ) ?>"><i
                        class="fa fa-pencil"></i><?php _e( "Edit", ET_DOMAIN ); ?></a>
		<?php } ?>
		...
		<?php if ( $post->post_status == 'publish' ) { ?>
            <a title="<?php _e( "Edit", ET_DOMAIN ); ?>" target="_Blank"
               href="<?php echo et_get_page_link( 'edit-project', [ 'id' => $post->ID ] ) ?>"><i
                        class="fa fa-pencil"></i><?php _e( "Edit", ET_DOMAIN ); ?></a>
            <a data-action="archive" class="action archive" href="#"><i
                        class="fa fa-trash-o"></i><?php _e( "Archive", ET_DOMAIN ); ?></a>
		<?php } ?>
		<?php if ( $post->post_status == 'draft' ) { ?>
            <a title="<?php _e( "Edit", ET_DOMAIN ); ?>" data-target="#" class="" target="_Blank"
               href="<?php echo et_get_page_link( 'submit-project', [ 'id' => $post->ID ] ) ?>">
		...
		<?php if ( $post->post_status == 'archive' ) { ?>
            <a title="<?php _e( "Renew", ET_DOMAIN ); ?>" data-target="#" class="" target="_Blank"
               href="<?php echo et_get_page_link( 'submit-project', [ 'id' => $post->ID ] ) ?>">
		...
		<?php if ( $post->post_status == 'reject' ) { ?>
            <a title="<?php _e( 'Edit', ET_DOMAIN ); ?>" target="_Blank"
               href="<?php echo et_get_page_link( 'edit-project', [ 'id' => $post->ID ] ) ?>">
                <i class="fa fa-pencil"></i><?php _e( "Edit", ET_DOMAIN ); ?>
            </a>
		<?php } ?>
	...
	function ae_js_edit_post_button() {
        ...
        <a href="<?php echo et_get_page_link( 'edit-project' ) ?>?id={{= ID }}" target="_Blank"
           title="<?php _e( 'Edit', ET_DOMAIN ); ?>"><i class="fa fa-pencil"></i><?php _e( 'Edit', ET_DOMAIN ); ?></a>
        <# } #>

        <# if(post_status == 'publish'){ #>
        <a href="<?php echo et_get_page_link( 'edit-project' ) ?>?id={{= ID }}" target="_Blank"
           title="<?php _e( 'Edit', ET_DOMAIN ); ?>"><i class="fa fa-pencil"></i><?php _e( 'Edit', ET_DOMAIN ); ?></a>
        <a data-action="archive" class="action archive" href="#" title="<?php _e( 'Archive', ET_DOMAIN ); ?>"><i
                    class="fa fa-trash-o"></i><?php _e( 'Archive', ET_DOMAIN ); ?></a>
        <# } #>
        ...
        <a title="<?php _e( 'Edit', ET_DOMAIN ); ?>"
           href="<?php echo et_get_page_link( 'edit-project' ) ?>?id={{= ID }}" target="_Blank"><i
                    class="fa fa-pencil"></i><?php _e( 'Edit', ET_DOMAIN ); ?></a>
        <# } #>
