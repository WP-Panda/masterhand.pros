...
class AE_PostMeta extends AE_Base {
	...
	public function add_meta_boxes() {
		add_meta_box( 'place_info', __( 'Project Information', ET_DOMAIN ), [
			$this,
			'meta_view'
	...
	public function add_meta_script() {
		...
		wp_enqueue_script( 'edit-ad', TEMPLATEURL . '/assets/js/edit-post.js', [
			'jquery',
		...
		wp_localize_script( 'edit-ad', 'edit_ad', [
	...
	public function meta_view( $post ) {
        ...
        <table class="form-table ad-info">
                ...
                <td>
					...
					if ( ! empty( $payment_package ) ) {
						foreach ( $payment_package as $key => $plan ) { ?>
                            <p>
                                <input data-duration="<?php echo $plan->et_duration ?>" class="ad-package"
                                       type="radio" id="et_ad_package_<?php
            ...
            <tr valign="top">
                <th scope="row"><label
                            for="et_featured"><strong><?php _e( "Featured post:", ET_DOMAIN ); ?></strong></label>
                </th>
                <td>
                    <input type="hidden" value="0" name="et_featured"/>
                    <input value="1" name="et_featured" type="checkbox"
                           id="et_featured" <?php checked( 1, $ad['et_featured'], true ); ?> >
                    <p class="description"><label
                                for="et_featured"><?php _e( "Make this post featured in listing.", ET_DOMAIN ); ?></label>
                    </p>
                </td>
            ...
            <tr valign="top">
                <th scope="row"><label
                            for="et_expired_date"><strong><?php _e( "Expired Date:", ET_DOMAIN ); ?></strong></label>
                </th>
                <td>
                    <input name="et_expired_date" type="text" id="et_expired_date"
                           value="<?php echo $ad['et_expired_date'] ?>" class="regular-text">
	...
	public function save_meta_fields( $post_id ) {
		...
		if ( ! isset( $_POST['_et_nonce'] ) || ! wp_verify_nonce( $_POST['_et_nonce'], $this->nonce ) ) {
			return;
		}
