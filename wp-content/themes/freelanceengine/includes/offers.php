...
function fre_register_offer()
{
    ...
    $ae_post_factory->set(ADVERT, new AE_Posts(ADVERT, array(), array(
        'country', 'state', 'city'
    )));
}
...
function special_offers_template_include($template)
{
    if (strpos($_SERVER['REQUEST_URI'], '/special-offers/') !== false) {
        ...
        add_filter('body_class', function ($classes) {
            return array_merge($classes, array('page-template-special_offers'));
        });
        status_header(200);
            ...
            foreach ($_FILES['files']['error'] as $ind => $error) {
                if ($error == 0 && $_FILES['files']['size'][$ind] <= 2100000) {
                    if ($countAttachments < $max_image) {
                        ...
                        $attach_id = attach_advert_file($prepare_attach, $advert_id, ['jpg|jpeg' => 'image/jpeg', 'png' => 'image/png',]);
...
function attach_advert_file($file, $parent = 0, $mimes = array(), $author = 0)
{
    global $user_ID;
