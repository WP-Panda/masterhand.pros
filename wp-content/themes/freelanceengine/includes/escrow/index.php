...
if(ae_get_option('use_escrow')) {
    ...
    require_once TEMPLATEPATH  . '/includes/payouts/PayPal/class-masterha-paypal-payout.php';
	require_once dirname(__FILE__) . '/ppadaptive.php';
	require_once dirname(__FILE__) . '/paypal.php';
}

function confirm_paypal_field() {
    $paid = false;
    $user_id = (int)$_GET["user"];
    $user = get_user_by('id', $user_id);
    if($user) {
        $execution = new FP_WC_PP_PayPal_Payment();
        $dataArr = $execution->execution($_GET);
        if($dataArr) {
            $paid = $dataArr->getState();
        }
        if($paid == 'approved') {
...
function fre_process_escrow($payment_type, $data) {
    ...
    if ($payment_type == 'paypaladaptive' || $payment_type == 'paypal') {
        // email confirm
        if ($paid == 'approved') {
            ...
            update_post_meta($data['order_id'], "PayerID", $_GET['PayerID']);
            ...
            if($post_status != 'publish') {
                ...
                $mailing->wp_mail($author_project_email, $subject, $message, array('post' => $project_id, 'user_id' => $author_project_id));
            }
