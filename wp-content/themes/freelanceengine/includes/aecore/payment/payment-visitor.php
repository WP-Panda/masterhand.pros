...
class ET_PaypalVisitor extends ET_PaymentVisitor {
	...
	function setup_checkout( ET_Order $order ) {
		...
		$payment  = new FP_WC_PP_PayPal_Payment( [
			"id"        => $order['ID'],
			"currency"  => $order['currencyCodeType'],
			"total"     => $order['total'],
			"returnUrl" => $settings['return'],
			"canselUrl" => $settings['cancel']
		] );
		...
		//var_dump($nvpstr);
		return array( 'url' => $approvalUrl, 'ACK' => $ASK, 'extend' => false );
	}
	...
	function setup_checkout( ET_Order $order ) {
		...
		if ( ! empty( $products ) ) {
			foreach ( $products as $key => $value ) {
				...
				$pro_str .= "&c_description_$k=" . urldecode( wp_strip_all_tags( $value['L_DESC'] ) );
