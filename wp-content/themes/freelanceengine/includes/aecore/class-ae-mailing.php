...
class AE_Mailing extends AE_Base {
	 ...
	 * @since  1.0
	 */
	public function wp_mail( $to, $subject, $content, $filter = [], $headers = '' ) {
		if ( $headers == '' ) {
	...
	function inbox_mail( $author, $inbox_message ) {
		...
		$this->wp_mail( $author->user_email, $subject, $message, [ 'user_id' => $current_user->ID ], $headers );
	}
	...
	function new_post_alert( $post ) {
		...
		$message = sprintf( __( "<p>Hi,</p><p> Have a new post on your site. You can review it here: %s </p>", ET_DOMAIN ), "<a style='color:#2C33C1' href='" . get_permalink( $post ) . "' target='_Blank'>" . get_the_title( $post ) . "</a>" );
