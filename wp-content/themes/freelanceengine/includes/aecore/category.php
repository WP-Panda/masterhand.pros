...
class AE_Term extends AE_Base {
	...
	public function create( $term, $args = [] ) {
	...
	public function delete( $id, $taxonomy, $default = false ) {
		if ( $default ) {
			$result = wp_delete_term( $id, $taxonomy, [ 'default' => $default ] );
		} else {
...
class AE_Category extends AE_Term {
	...
	public function __construct( $args = [] ) {
		$args = wp_parse_args( $args, [ 'taxonomy' => 'place_category', 'label' => 'Place Category' ] );
		...
		$this->icons_list = isset( $args['icons_list'] ) ? $args['icons_list'] : [];
...
class AE_BackendCategory extends AE_Category {
	...
	function print_backend_terms( $parent = 0, $positions = false ) {
		...
		?>
        <ul class="list-job-input list-tax category list-job-categories cat-sortable tax-sortable" id="place_cats"
            data-tax="<?php echo $this->taxonomy ?>">
			<?php
                ...
                <div class="container">
                    ...
                    <div class="input-form input-form-1 color-default">
                                ...
                                <div class="color-panel" style="display:none;">
									...
									foreach ( $arr_colors as $key => $value ) { ?>
                                        <div class="color-item" data="<?php echo $value; ?>">
                                                    <span style="background-color:<?php echo $value ?>"
                                                          class="flags"></span>
                                        </div>
									<?php } ?>
                                    <div class="custom-color"><label>Set your custom color</label><input
                                                class="input-color color-picker " placeholder="e.g: #fbfbfb"
                                                value=""/></div>
                                </div>
                            </div>
						<?php } ?>
                        ...
                        <input style="color:#000000;"
                               class="bg-grey-input tax-name <?php echo $class_no_icon . ' ' . $class_no_color ?>"
                               name="name" data-tax="<?php echo $this->taxonomy ?>" autocomplete="off"
                               placeholder="<?php _e( 'Add a category', ET_DOMAIN ) ?>" type="text"/>
                    </div>
                </div>
            </li>
        </ul>
	...
	function print_backend_terms_li( $parent = 0, $positions = false ) {
		...
		if ( ! $positions ) {
			$positions = $this->getAll( [ 'taxonomy' => $this->taxonomy ] );
		}
		foreach ( $positions as $job_pos ) {
			if ( $job_pos->parent == $parent ) {
				?>
                <li class="tax-item" data-id="<?php echo $job_pos->term_id ?>"
                    data-tax="<?php echo $this->taxonomy ?>"
                    data-color="<?php echo isset( $colors[ $job_pos->term_id ] ) ? $colors[ $job_pos->term_id ] : '#000' ?>"
                    data-icon="fa-map-marker" id="tax_<?php echo $job_pos->term_id ?>">
                    <div class="container">
                        <div class="<?php echo $class_no_hierarchical; ?> "></div>
                        <div class="controls controls-2">
                            <a class="button act-open-form" rel="<?php echo $job_pos->term_id ?>"
                               title="<?php _e( 'Add sub tax for this tax', ET_DOMAIN ) ?>">
                        ...
                        <div class="input-form input-form-1"
                             data-action="et_update_<?php echo $this->taxonomy ?>_color">
							<?php if ( $this->is_use_color ) { ?>
                                <div class="cursor">
                                        <span class="flag"
                                              style="background-color:<?php echo isset( $colors[ $job_pos->term_id ] ) ? $colors[ $job_pos->term_id ] : '#000' ?>;"></span>
                                    <div class="color-panel" style="display:none;">
										<?php $arr_colors = [
											'#000000',
											'#1abc9c',
							...
							<?php if ( $this->is_use_icon ) { ?>
                                <div class="icon trigger"
                                     data="<?php echo isset( $icons[ $job_pos->term_id ] ) ? $icons[ $job_pos->term_id ] : 'fa-map-marker'; ?>">
                                    <i class="fa <?php echo isset( $icons[ $job_pos->term_id ] ) ? $icons[ $job_pos->term_id ] : 'fa-map-marker'; ?>"></i>
                                </div>
							<?php } ?>
                            <input style="color:<?php echo isset( $colors[ $job_pos->term_id ] ) ? $colors[ $job_pos->term_id ] : '#000' ?>;"
                                   class="bg-grey-input tax-name <?php echo $class_no_icon . ' ' . $class_no_color ?>"
                                   name="name" autocomplete="off" rel="<?php echo $job_pos->term_id ?>" type="text"
                                   value="<?php echo $job_pos->name ?>">
                        </div>
                    </div>
                    <ul>
	...
	function print_confirm_list() {
		if ( ! is_array( $this->_term_in_order ) ) {
			$this->getAll( [ 'taxonomy' => $this->taxonomy ] );
		}
		?>
		...
		try {
			...
			switch ( $method ) {
				case 'delete':
					...
					$children = get_terms( $taxonomy, [ 'parent' => $term, 'hide_empty' => false ] );
				...
				case 'create' :
					...
					$args = [ 'color' => 0, 'icon' => 'fa-map-marker' ];
				...
				case 'sort':
					...
					$order  = $order['tax'];
					$handle = new AE_Category( [ 'taxonomy' => $taxonomy ] );
					$handle->sort( $order, [ 'taxonomy' => $taxonomy ] );
			...
			$resp = build_error_ajax_response( [], $e->getMessage() );
		}
		wp_send_json( $resp );
	}
}
