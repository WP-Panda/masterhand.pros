...
class AE_Overview extends AE_Page {
	...
	public function overview_scripts() {
		...
		$this->add_script( 'jqplot', ae_get_url() . '/assets/js/jquery.jqplot.min.js', [
			'jquery'
		] );
		$this->add_script( 'jqplot-plugins', ae_get_url() . '/assets/js/jqplot.plugins.js', [
			'jquery',
			'jqplot'
		] );

		$this->add_script( 'ae-overview', ae_get_url() . '/assets/js/overview.js', [
			'jquery',
			'jqplot',
			'appengine'
		] );
