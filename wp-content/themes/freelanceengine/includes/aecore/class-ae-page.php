...
class AE_Page extends AE_Base {
	...
	function sub_menu_page() {
		$args = $this->args;
		add_submenu_page( $args['parent_slug'], $args['page_title'], $args['page_title'], 'manage_options', $args['slug'], [
			$this,
			'render_frame'
		] );
	}
	...
	public function print_scripts() {
		...
		if ( isset( $_GET['page'] ) && $_GET['page'] == "et-users" ) {
			$this->add_script( 'bootstrap-js', ae_get_url() . '/assets/js/bootstrap.min.js', [
				'jquery'
			] );
		}
		...
		$this->add_script( 'lib-nested-sortable', ae_get_url() . '/assets/js/jquery.nestedSortable.js', [
			'jquery',
			'jquery-ui-sortable'
		] );
		// tam thoi add de xai
		...
		$this->add_script( 'ae-colorpicker', ae_get_url() . '/assets/js/colorpicker.js', [
			'jquery'
		] );

		$this->add_script( 'marionette', ae_get_url() . '/assets/js/marionette.js', [
			'jquery',
			'underscore',
			'backbone'
		] );
		...
		$this->add_script( 'appengine', ae_get_url() . '/assets/js/appengine.js', [
			'jquery',
			'underscore',
			'backbone',
			'marionette',
		...
		// control backend user list
		$this->add_script( 'backend-user', ae_get_url() . '/assets/js/user-list.js', [
			'appengine'
		] );

		// control backend order list
		$this->add_script( 'order-list', ae_get_url() . '/assets/js/payment-list.js', [
			'appengine'
		] );

		//  option settings and save
		$this->add_script( 'option-view', ae_get_url() . '/assets/js/option-view.js', [
			'appengine'
		] );

		// control option translate
		$this->add_script( 'language-view', ae_get_url() . '/assets/js/language-view.js', [
			'appengine',
			'option-view'
		] );

		// control pack view add delete update pack
		$this->add_script( 'pack-view', ae_get_url() . '/assets/js/pack-view.js', [
			'appengine',
			'option-view'
		] );

		// backend js it should be separate by theme
		$this->add_script( 'backend', ae_get_url() . '/assets/js/backend.js', [
			'appengine'
		] );
		$max_upload_size = wp_max_upload_size();
		$vars            = apply_filters( 'ae_admin_globals', [
			...
			'texts'            => [ 'limit_category_level' => __( "Categories' level is limited to 3", ET_DOMAIN ) ],
