...
if ( ! class_exists( 'AE_Base' ) ) {
	class AE_Base {
		 ...
		 * @param       $in_footer
		 */
		public function add_script( $handle, $src, $deps = [], $ver = false, $in_footer = true ) {
			$scripts = apply_filters( self::FILTER_SCRIPT, [
				'handle'    => $handle,
				'src'       => $src,
				'deps'      => $deps,
				'ver'       => $ver,
		 ...
		 * @param       $in_footer
		 */
		public function add_existed_script( $handle, $src = '', $deps = [], $ver = false, $in_footer = true ) {
		 ...
		 * @param       $media
		 */
		public function add_style( $handle, $src = false, $deps = [], $ver = false, $media = 'all' ) {
			$style = apply_filters( self::FILTER_STYLE, [
				'handle' => $handle,
				'src'    => $src,
				'deps'   => $deps,
				'ver'    => $ver,
		 ...
		 * @param       $media
		 */
		public function register_style( $handle, $src = false, $deps = [], $ver = false, $media = 'all' ) {
			$style = apply_filters( self::FILTER_STYLE, [
				'handle' => $handle,
				'src'    => $src,
				'deps'   => $deps,
