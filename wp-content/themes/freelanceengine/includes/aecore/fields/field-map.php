...
class AE_map {
	...
	function render() {
		...
		extract( $value );
		?>
        <div class="map-setting" id="map-setting-<?php echo $this->field['id']; ?>"
             data-name="<?php echo $this->field['name'] ?>">
            <input value="<?php echo $address; ?>" type="text" class="address clearfix" name="address"
                   placeholder="<?php _e( "Enter map address", ET_DOMAIN ); ?>"/>
			...
			foreach ( $style_list as $key => $value ) {
				?>
                <option data-code='<?php echo $value['code'] ?>'
                        value="<?php echo $key ?>" <?php selected( $key, $this->value['style'] ); ?>>
					<?php echo $value['name'] ?>
                </option>
				<?php
			}
