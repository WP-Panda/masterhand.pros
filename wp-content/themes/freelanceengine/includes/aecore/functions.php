...
static $idsUsersRoles = [];
function userRole( $userId = '' ) {
	global $idsUsersRoles;
	if ( empty( $userId ) ) {
		return false;
	}

	if ( isset( $idsUsersRoles[ $userId ] ) ) {
		return $idsUsersRoles[ $userId ];
	} else {
		$role                     = ae_user_role( $userId );
		$r                        = $role == 'administrator' ? EMPLOYER : $role;
		$idsUsersRoles[ $userId ] = $r;
 ...
 *
 * @return mixed       Post object if successful. Null if global $post is not set. Empty string if no corresponding
 *                     post exists.
 *
 * @category ulti
 *
...
function ae_editor_settings() {
	return apply_filters( 'ae_editor_settings', [
		'quicktags'     => false,
		'media_buttons' => false,
		'wpautop'       => false,
 ...
 * @author anhcv
 */
function et_process_file_upload( $file, $author = 0, $parent = 0, $mimes = [] ) {
...
if ( ! function_exists( 'et_get_attachment_data' ) ):
	...
	function et_get_attachment_data( $attach_id, $size = [] ) {
...
function et_load_tablet() {
	...
	if ( $isTablet /*&& (!isset($_COOKIE['mobile']) || md5('disable') != $_COOKIE['mobile'] )*/ ) {
		return true;
	} else {
		return false;
	}
...
if ( ! function_exists( 'ae_get_social_login_user_roles_default' ) ) {
	function ae_get_social_login_user_roles_default() {
		return apply_filters( 'ae_social_login_user_roles_default', [ 'author' ] );
	}
}
/**
...
function et_change_user_role() {
	$employers   = [ 'jazminewinton', 'kermitlawrence', 'hatruong', 'dorisclarke' ];
	$freelancers = [ 'hamletavakian', 'taikiyamada', 'fadilaahmed', 'morganmelvin', 'martinharvey' ];
