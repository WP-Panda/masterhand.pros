...
class AE_Package extends AE_Pack {
	...
	function __construct( $post_type = 'pack', $meta = [] ) {
	...
	public function get_pack( $sku = '', $pack_post_type = '' ) {
		...
		if ( empty( $options->$option_name ) && in_array( $pack_post_type, [
				'fre_credit_plan',
				'bid_plan'
			] ) ) { // 1.8.6 fix bug pay bid
	...
	public static function check_use_package( $sku, $user_id = 0 ) {
		...
		if ( ! $order || is_wp_error( $order ) || ! in_array( $order->post_status, [ 'pending', 'publish' ] ) ) {
			return false;
		}
	...
	public static function package_or_free( $package_id, $ad ) {
		...
		if ( $use_package ) {
			...
			$response['url']     = et_get_page_link( 'process-payment', [
