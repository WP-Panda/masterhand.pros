                    ...
                    <p>
						<?php
							...
							if ( $order->post_status == 'publish' ) {
								printf( __( "The <strong>%s</strong> package you purchased has %d post(s) left", ET_DOMAIN ), $package->post_title, $number_of_post );
							}
							if ( $order->post_status == 'pending' ) {
								printf( __( "The <strong>%s</strong> package you purchased has %d post(s) left. Your posted post is pending until the payment is processed.", ET_DOMAIN ), $package->post_title, $number_of_post );
							}
						?>
                    </p>
		...
		foreach ( $packs as $package ) {
			...
			if ( isset( $package_data[ $sku ] ) ) {
				...
				if ( isset( $package_data[ $sku ][ 'qty' ] ) && $package_data[ $sku ][ 'qty' ] > 0 ) {
					if ( $package->post_type == 'pack' ) {
						$order = get_post( $orders[ $sku ] );
						if ( ! $order || is_wp_error( $order ) || ! in_array( $order->post_status, [ 'publish' ] ) ) {
							continue;
						}
						$number_of_post = $package_data[ $sku ][ 'qty' ];
