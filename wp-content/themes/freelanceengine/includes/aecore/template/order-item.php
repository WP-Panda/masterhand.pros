...
<li>
    <div class="method">
		...
		if ( $post->post_status == 'pending' ) : ?>
            <a title="<?php _e( "Approve", ET_DOMAIN ); ?>" class="color-green action publish"
               data-id="<?php echo $post->ID; ?>" href="#">
                <span class="icon" data-icon="3"></span>
            </a>
            <a title="<?php _e( "Decline", ET_DOMAIN ); ?>" class="color-red action decline"
               data-id="<?php echo $post->ID; ?>" href="#">
    ...
    <div class="content">
		...
		if ( $post ) {
			...
			_e( ' by ', ET_DOMAIN );
			?>
            <a target="_blank" href="<?php echo get_author_posts_url( $post->post_author, $author_nicename = '' ) ?>"
               class="company">
