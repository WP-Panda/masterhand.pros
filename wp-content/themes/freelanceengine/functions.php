...
//    $person .= "\n".date('d.m.Y h:i:s')."==============GET PARAMETERS==============\n".print_r($_GET, true);

//}

//if($_POST) {

//    $person .= "\n".date('d.m.Y h:i:s')."==============POST PARAMETERS==============\n".print_r($_POST, true);

//}

//if($_REQUEST) {

//    $person .= "\n".date('d.m.Y h:i:s')."==============REQUEST PARAMETERS==============\n".print_r($_REQUEST, true);

//}

//if($_GET || $_POST || $_REQUEST)

//    $ok = file_put_contents($_SERVER["DOCUMENT_ROOT"]."/log.txt", $person, FILE_APPEND);
...

{
...

    {
        ...
        if (!isset($wp_roles->roles[FREELANCER])) {
            ...
            add_role(FREELANCER, __('Professional', ET_DOMAIN), array(
        ...
        if (!isset($wp_roles->roles[EMPLOYER])) {
            ...
            add_role(EMPLOYER, __('Client', ET_DOMAIN), array(
...

    {
        ...
        $role = array(FREELANCER => __('Professional', ET_DOMAIN), EMPLOYER => __('Client', ET_DOMAIN));
...

    {
        ...
        if ($role == FREELANCER) {
            ...
            $redirect_url = et_get_page_link('profile', array('loginfirst' => 'true')) . '#settings';
...

    {
        ...
        if (current_user_can('administrator')) {
            ...
            wp_enqueue_script('admin_additional', get_template_directory_uri().'/assets/js/admin_additional.js', array('jquery'));
...

    {
        ...
        */
        ...
        $this->add_script('maskedinput', get_template_directory_uri() . '/js/jquery.maskedinput.min.js', array(
        ...
        $this->add_script('custom-scripts', get_template_directory_uri() . '/js/custom-scripts.js', array(
        ...
        */
        ...
        if (!is_user_logged_in()) {
            ...
            wp_enqueue_script('confirm_paypal', get_template_directory_uri() . '/assets/js/paypal_confirmation.js', array(), time(), false);
        ...
        if (et_load_mobile()) {
            ...
            */
            ...
            if (is_page_template('page-submit-project.php') ||
                ...
                is_page_template('page-upgrade-account.php') ||

                is_page_template('page-listing-plan.php') ||

                is_page_template('page-pro-order-payment.php'))
...

    {
        ...
        $this->add_style('custom1', get_template_directory_uri() . '/css/custom.css', array(), ET_VERSION);
...

{
    ...
    if (!empty($document_list)) {
        ...
        $documents = is_numeric($document_list) ? array($document_list) : explode(', ', $document_list);
...

{
    ...
    if (!empty($_POST["country_id"])) {
        ...
        $results = $wpdb->get_results(
            ...
            $wpdb->prepare("SELECT `id`, `name` FROM {$wpdb->prefix}location_states WHERE `country_id` = %d ORDER BY `name`", $_POST["country_id"]), OBJECT
    ...
    if (!empty($_POST["state_id"])) {
        ...
        $results = $wpdb->get_results(
            ...
            $wpdb->prepare("SELECT `id`, `name` FROM {$wpdb->prefix}location_cities WHERE `state_id` = %d ORDER BY `name`", $_POST["state_id"]), OBJECT
...

{
    ...
    if (!empty($res["term_id"])) {
        ...
        ae_tax_dropdown($type_filter, array(

            'attr' => '',//'data-chosen-width="100%" data-chosen-disable-search="" data-placeholder="' . __( "Select sub1", ET_DOMAIN ) . '"',
...

{
    ...
    if (empty($_POST) || empty($img_src = $_POST["img_src"])) echo '';
    ...
    if (!empty($emails = $_POST['emails'])) {
        ...
        $res_send = wp_mail($emails, $subject, $message, '', array($img_src));
        ...
        echo $mes = array('status' => 'error', 'msg' => 'no emails');
...

{
    ...
    echo json_encode(array('msg' => $wp_error->errors['wp_mail_failed']));
...
function getLocation($id = 0, $location_id = array('country' => '', 'state' => '', 'city' => ''))
...

{
    ...
    wp_enqueue_script(
        ...
        get_stylesheet_directory_uri() . '/js/jquery.jscrollpane.js',
...
//        $andExpireDate = "AND pp.expired_date > UNIX_TIMESTAMP()";
        ...
        if (!empty($data_emails)) {
            ...
            $message = '<p>' . __('On website {{site_url}} published new project for your relevant skills:') . '</p>'

                . '<br>' . '<b>{{title_project}}</b>'

                . '<br>' . __('You can view it from this link: ') . '<a href="{{link_project}}">{{link_project}}</a>'

                . '<br><br>' . __('Thanks & Regards, Admin');
    ...
    if(!is_admin()){
        ...
        wp_localize_script('jquery', 'ajax_var', // добавляем объект с глобальными JS переменными
            ...
            array(
                ...
                'url' => admin_url('admin-ajax.php'), // и суем в него путь до AJAX обработчика

                'nonce' => wp_create_nonce('ajax_object') // передадим уникальную строку для механизма проверки аякс запроса, ajax_var.nonce
...
function checking_email_hash() {
    ...
    if($_GET['email_hash'] && $_GET['user_id'] > 0 && !is_user_logged_in()) {
...
add_action( 'restrict_manage_posts', function(){
    ...
    if ('post' == $type){
        ...
        <select name="author_type">
            ...
            <option value="masterhand" <?php echo $current_value == 'masterhand' ? 'selected' : ''?>>Masterhand</option>
    ...
    <div class="usp_attaches__container">
        ...
        <ul class="freelance-portfolio-list row">
            ...
            <?php foreach ($usp_attaches as $usp_attach){
                ...
                $document = ['id' => $doc_data->ID, 'url' => wp_get_attachment_url($doc_data->ID), 'mime' => $doc_data->post_mime_type, 'title' => $doc_data->post_title];
                ...
                <li class="col-sm-4 col-md-3 col-lg-3 col-sx-12" id="usp_attach_item_<?php echo $document['id']?>">
                    ...
                    >
                        ...
                        <div class="portfolio-action">
                            ...
                            <a href="<?=$document['url']?>" target="_blank" class="fre-submit-btn btn-center" href="<?php echo $document['url']?>"><?php _e('Open', ET_DOMAIN) ?></a>



                            <a href="javascript:void(0)" class="fre-cancel-btn btn-center" onclick="attach_remove_modal(<?php echo $post->ID?>, <?php echo $document['id']?>)">
        ...
        <div class="modal-dialog modal-sm">
            ...
            <div class="modal-content">
                ...
                <div class="modal-header">
                    ...
                    <button type="button" class="close" data-dismiss="modal" onclick="jQuery('#modal_delete_file').hide()"></button>
                ...
                <div class="modal-body">
                    ...
                    <div class="fre-modal-form form_delete_file" id="form_delete_file">
                        ...
                        <div class="fre-form-btn">
                            ...
                            <button class="fre-submit-btn btn-left btn_submit_document" type="button" onclick="attach_remove()"><?php _e('Confirm', ET_DOMAIN) ?></button>



                            <span class="fre-cancel-btn" onclick="jQuery('#modal_delete_file').hide()" data-dismiss="modal"><?php _e('Cancel', ET_DOMAIN) ?></span>
...
function mh_export_additional_wpae() {
    ...
    if ( $screen->id != 'all-export_page_pmxe-admin-manage' ) return;
...
function register_faq_post_type() {
    ...
    register_taxonomy('faqcat', array('faq'), array(
        ...
        'label'                 => 'Question section', // определяется параметром $labels->name
        ...
        'description'           => 'Headings for the question section', // описание таксономии
    ...
    register_post_type('faq', array(
        ...
        'rewrite'             => array( 'slug'=>'help/%faqcat%', 'with_front'=>false, 'pages'=>false, 'feeds'=>false, 'feed'=>false ),
...
if ( 'activated' == $query->get( 'orderby' ) ) {
    $query->set( 'orderby', array( 'meta_value', 'user_registered' => 'ASC') );
...
// function export_unsubscribe_users($wp_admin_bar){
// $args = array(
// 'title' => 'Export Unsubscribed Users',
// 'href' => '/wp-content/themes/freelanceengine/export_unsubscribe_users.php?data="xpCbBoXR40Y4rORgIjRGVoK4an6lvP20mmrlxN4hOTGtm2mXwOJywCPY3dsIxMcKHlotRF"',
// 'meta' => array(
...
// add_action('admin_bar_menu', 'export_unsubscribe_users', 50);