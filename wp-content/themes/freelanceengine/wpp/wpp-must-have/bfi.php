	...
	class BFI_Thumb_1_3 {
		...
		public static function thumb( $url, $params = [], $single = true ) {
			...
			if ( strpos( $url, $upload_url ) !== false ) {
				...
				// #3 check if the image is in the plugins folder
			} else if ( strpos( $url, WP_PLUGIN_URL ) !== false ) {
				$rel_path = str_replace( WP_PLUGIN_URL, '', $url );
				$img_path = WP_PLUGIN_DIR . $rel_path;
			}
			...
			$first  = ! empty( $width ) ? $width : '';
			$larst  = ! empty( $height ) ? $height : '';
			$middle = ! empty( $first ) && ! empty( $larst ) ? 'x' : '';

			$suffix = $first . $middle . $larst;

			//$past = ! empty( $water_mark ) ? '-wwm' : '';
			//if ( ! empty( $optimize ) ) {
			//	$past .= class_exists( 'ArtisansWeb\Optimizer' ) ? '-io-n' : '';
			//}
			...
			if ( defined( 'BFITHUMB_UPLOAD_DIR' ) ) {
				...
				$upload_dir .= "/wpp_thumb";
				$upload_url .= "/wpp_thumb";
			}
			...
			$destfilename = "{$upload_dir}/{$dst_rel_path}/{$dst_rel_path}_{$suffix}.{$ext}";

			// The urls generated have lower case extensions regardless of the original case
			$ext     = strtolower( $ext );
			$img_url = "{$upload_url}/{$dst_rel_path}/{$dst_rel_path}_{$suffix}.{$ext}";
			...
			if ( @file_exists( $destfilename ) && getimagesize( $destfilename ) ) {
				...
				if ( ! empty( $water_mark ) && is_callable( [ $editor, 'stamp_watermark' ] ) ) {
					$editor->stamp_watermark( ABSPATH . 'wp-content/plugins/wpp-framework/assets/images/wm.png', ( int ) $editor->get_size()['width'] / 2 - 180, 30 );
				}
...
if ( ! function_exists( 'bfi_image_downsize' ) ) {
	function bfi_image_downsize( $out, $id, $size ) {
		...
		if ( empty( $img_url ) ) {
			$img_url = wpp_fr_image_placeholder();
		}
...
add_filter( 'wp_image_editors', function ( $editors ) {
	if ( ! is_array( $editors ) ) {
		return $editors;
	} //someone broke the filtered value
