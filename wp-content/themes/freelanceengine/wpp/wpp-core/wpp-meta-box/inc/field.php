...
abstract class WPP_MB_Field {
	...
	public static function show( $field, $saved, $post_id = 0 ) {
		...
		if ( $field['clone'] ) {
			$field_html = WPP_MB_Clone::html( $meta, $field );
		} else {
	...
	public static function end_html( $meta, $field ) {
		return WPP_MB_Clone::add_clone_button( $field ) . self::call( 'input_description', $field ) . '</div>';
	}
	 ...
	 * @param array $field     Field parameters.
	 * @param array $args      Arguments of {@see wpp_mb_meta()} helper.
	 *
	 * @return mixed
	 */
	public static function raw_meta( $object_id, $field, $args = array() ) {
		...
		if ( isset( $field['storage'] ) ) {
			...
			$storage = wpp_mb_get_storage( $args['object_type'] );
		} else {
			$storage = wpp_mb_get_storage( 'post' );
		}
	...
	public static function save( $new, $old, $post_id, $field ) {
		...
		if ( ! WPP_MB_Helpers_Value::is_valid_for_field( $new ) ) {
	...
	public static function render_attributes( $attributes ) {
		...
		$attributes = array_filter( $attributes, 'WPP_MB_Helpers_Value::is_valid_for_attribute' );
	 ...
	 * wpp_mb_the_field function later.
	 *
	 * @use self::get_value()
	 * @see wpp_mb_the_value()
	 *
	 * @param  array    $field   Field parameters.
	 * @param  array    $args    Additional arguments. Rarely used. See specific fields for details.
	 * @param  int|null $post_id Post ID. null for current post. Optional.
	...
	public static function call() {
		...
		return call_user_func_array( array( WPP_MB_Helpers_Field::get_class( $field ), $method ), $args );
	}
	 ...
	 * Filters:
	 * - wpp_mb_{$name}
	 * - wpp_mb_{$field['type']}_{$name}
	 * - wpp_mb_{$field['id']}_{$name}
	 *
	 * @return mixed
	 */
	public static function filter() {
		...
		if ( isset( $field['id'] ) ) {
			$filters[] = 'wpp_mb_' . $field['id'] . '_' . $name;
		}
