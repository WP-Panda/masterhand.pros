...
class WPPMB_Meta_Box {
	...
	public function __construct( $meta_box ) {
		...
		$this->meta_box = apply_filters( 'wpp_mb_meta_box_settings', $this->meta_box );
	...
	public function register_fields() {
		$field_registry = wpp_mb_get_registry( 'field' );
	...
	public function is_shown() {
		$show = apply_filters( 'wpp_mb_show', true, $this->meta_box );

		return apply_filters( "wpp_mb_show_{$this->id}", $show, $this->meta_box );
	}
	...
	protected function global_hooks() {
		...
		foreach ( $this->fields as $field ) {
			WPP_MB_Field::call( $field, 'add_actions' );
		}
	}
	...
	public function enqueue() {
		...
		wp_enqueue_style( 'rwmb', WPP_MB_CSS_URL . 'style.css', array(), WPP_MB_VER );
		if ( is_rtl() ) {
			wp_enqueue_style( 'rwmb-rtl', WPP_MB_CSS_URL . 'style-rtl.css', array(), WPP_MB_VER );
		}

		// Load clone script conditionally.
		foreach ( $this->fields as $field ) {
			if ( $field['clone'] ) {
				wp_enqueue_script( 'rwmb-clone', WPP_MB_JS_URL . 'clone.js', array( 'jquery-ui-sortable' ), WPP_MB_VER, true );
				break;
			}
		}
		...
		foreach ( $this->fields as $field ) {
			WPP_MB_Field::call( $field, 'admin_enqueue_scripts' );
		}

		// Auto save.
		if ( $this->autosave ) {
			wp_enqueue_script( 'rwmb-autosave', WPP_MB_JS_URL . 'autosave.js', array( 'jquery' ), WPP_MB_VER, true );
		}
		 ...
		 */
		do_action( 'wpp_mb_enqueue_scripts', $this );
	}
	...
	public function show() {
		...
		do_action( 'wpp_mb_before', $this );
		do_action( "wpp_mb_before_{$this->id}", $this );

		foreach ( $this->fields as $field ) {
			WPP_MB_Field::call( 'show', $field, $saved, $this->object_id );
		}

		// Allow users to add custom code after meta box content.
		// 1st action applies to all meta boxes.
		// 2nd action applies to only current meta box.
		do_action( 'wpp_mb_after', $this );
		do_action( "wpp_mb_after_{$this->id}", $this );
	...
	public function save_post( $object_id ) {
		...
		// Before save action.
		do_action( 'wpp_mb_before_save_post', $object_id );
		do_action( "wpp_mb_{$this->id}_before_save_post", $object_id );

		array_map( array( $this, 'save_field' ), $this->fields );

		// After save action.
		do_action( 'wpp_mb_after_save_post', $object_id );
		do_action( "wpp_mb_{$this->id}_after_save_post", $object_id );
	}
	...
	public function save_field( $field ) {
		...
		$old    = WPP_MB_Field::call( $field, 'raw_meta', $this->object_id );
		// @codingStandardsIgnoreLine
		$new = isset( $_POST[ $field['id'] ] ) ? $_POST[ $field['id'] ] : ( $single ? '' : array() );

		// Allow field class change the value.
		if ( $field['clone'] ) {
			$new = WPP_MB_Clone::value( $new, $old, $this->object_id, $field );
		} else {
			$new = WPP_MB_Field::call( $field, 'value', $new, $old, $this->object_id );
			$new = WPP_MB_Field::filter( 'sanitize', $new, $field );
		}
		$new = WPP_MB_Field::filter( 'value', $new, $field, $old );

		// Filter to allow the field to be modified.
		$field = WPP_MB_Field::filter( 'field', $field, $field, $new, $old );

		// Call defined method to save meta value, if there's no methods, call common one.
		WPP_MB_Field::call( $field, 'save', $new, $old, $this->object_id );

		WPP_MB_Field::filter( 'after_save_field', null, $field, $new, $old, $this->object_id, $field );
	}
	...
	public static function normalize( $meta_box ) {
		...
		$meta_box = wp_parse_args(
			$meta_box,
			array(
				'id'             => ! empty( $meta_box['title'] ) ? sanitize_title( $meta_box['title'] ) : '',
				'context'        => 'normal',
				'priority'       => 'high',
				'post_types'     => 'post',
				'autosave'       => false,
		 ...
		 * @since 4.4.1
		 */
		WPP_MB_Helpers_Array::change_key( $meta_box, 'pages', 'post_types' );

		// Make sure the post type is an array and is sanitized.
		$meta_box['post_types'] = array_map( 'sanitize_key', WPP_MB_Helpers_Array::from_csv( $meta_box['post_types'] ) );
	 ...
	 * @param WPP_MB_Storage_Interface $storage Storage object. Optional.
	 *
	...
	public static function normalize_fields( $fields, $storage = null ) {
		foreach ( $fields as $k => $field ) {
			...
			$field = apply_filters( 'wpp_mb_normalize_field', $field );
			$field = apply_filters( "wpp_mb_normalize_{$type}_field", $field );
			$field = apply_filters( "wpp_mb_normalize_{$field['id']}_field", $field );
	...
	public function is_saved() {
		foreach ( $this->fields as $field ) {
			...
			$value = WPP_MB_Field::call( $field, 'raw_meta', $this->object_id );
			if ( false === $value ) {
				continue;
			}
			if (
	...
	public function get_storage() {
		return wpp_mb_get_storage( $this->object_type, $this );
	}
