...
class WPP_MB_Media_Modal {
	...
	public function init() {
		...
		add_filter( 'wpp_mb_show', array( $this, 'is_in_normal_mode' ), 10, 2 );
	}
	...
	public function get_fields() {
		$meta_boxes = wpp_mb_get_registry( 'meta_box' )->all();
	...
	public function add_fields( $form_fields, WP_Post $post ) {
		foreach ( $this->fields as $field ) {
			...
			$meta                = WPP_MB_Field::call( $field, 'meta', $post->ID, true );
			$form_field['value'] = $meta;

			$field['field_name'] = 'attachments[' . $post->ID . '][' . $field['field_name'] . ']';
			$form_field['html']  = WPP_MB_Field::call( $field, 'html', $meta );
	...
	public function save_fields( $post, $attachment ) {
		foreach ( $this->fields as $field ) {
			...
			$old = WPP_MB_Field::call( $field, 'raw_meta', $post['ID'] );
			$new = isset( $attachment[ $key ] ) ? $attachment[ $key ] : '';

			// Allow field class change the value.
			if ( $field['clone'] ) {
				$new = WPP_MB_Clone::value( $new, $old, $post['ID'], $field );
			} else {
				$new = WPP_MB_Field::call( $field, 'value', $new, $old, $post['ID'] );
				$new = WPP_MB_Field::filter( 'sanitize', $new, $field );
			}
			$new = WPP_MB_Field::filter( 'value', $new, $field, $old );

			// Call defined method to save meta value, if there's no methods, call common one.
			WPP_MB_Field::call( $field, 'save', $new, $old, $post['ID'] );
		}
