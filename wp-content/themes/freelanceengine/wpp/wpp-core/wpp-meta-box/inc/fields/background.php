 ...
 * The Background field.
 */
class WPP_MB_Background_Field extends WPP_MB_Field {
	/**
	 * Enqueue scripts and styles.
	 */
	public static function admin_enqueue_scripts() {
		wp_enqueue_style( 'rwmb-background', WPP_MB_CSS_URL . 'background.css', '', WPP_MB_VER );

		WPP_MB_Color_Field::admin_enqueue_scripts();
		WPP_MB_File_Input_Field::admin_enqueue_scripts();
	}
	...
	public static function html( $meta, $field ) {
		...
		// Color.
		$color   = WPP_MB_Color_Field::normalize(
			array(
				'type'       => 'color',
				'id'         => "{$field['id']}_color",
		...
		$output .= WPP_MB_Color_Field::html( $meta['color'], $color );

		$output .= '</div><!-- .rwmb-background-row -->';
		$output .= '<div class="rwmb-background-row">';

		// Image.
		$image   = WPP_MB_File_Input_Field::normalize(
			array(
				'type'        => 'file_input',
				'id'          => "{$field['id']}_image",
		...
		$output .= WPP_MB_File_Input_Field::html( $meta['image'], $image );

		$output .= '</div><!-- .rwmb-background-row -->';
		$output .= '<div class="rwmb-background-row">';

		// Repeat.
		$repeat  = WPP_MB_Select_Field::normalize(
			array(
				'type'        => 'select',
				'id'          => "{$field['id']}_repeat",
		...
		$output .= WPP_MB_Select_Field::html( $meta['repeat'], $repeat );

		// Position.
		$position = WPP_MB_Select_Field::normalize(
			array(
				'type'        => 'select',
		...
		$output  .= WPP_MB_Select_Field::html( $meta['position'], $position );

		// Attachment.
		$attachment = WPP_MB_Select_Field::normalize(
			array(
				'type'        => 'select',
		...
		$output    .= WPP_MB_Select_Field::html( $meta['attachment'], $attachment );

		// Size.
		$size    = WPP_MB_Select_Field::normalize(
			array(
				'type'        => 'select',
				'id'          => "{$field['id']}_size",
		...
		$output .= WPP_MB_Select_Field::html( $meta['size'], $size );
		$output .= '</div><!-- .rwmb-background-row -->';
