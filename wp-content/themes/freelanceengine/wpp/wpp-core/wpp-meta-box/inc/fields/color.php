 ...
 * Color field class.
 */
class WPP_MB_Color_Field extends WPP_MB_Text_Field {
	/**
	 * Enqueue scripts and styles.
	 */
	public static function admin_enqueue_scripts() {
		wp_enqueue_style( 'rwmb-color', WPP_MB_CSS_URL . 'color.css', array( 'wp-color-picker' ), WPP_MB_VER );

		$dependencies = array( 'wp-color-picker' );
		$args         = func_get_args();
		$field        = reset( $args );
		if ( ! empty( $field['alpha_channel'] ) ) {
			wp_enqueue_script( 'wp-color-picker-alpha', WPP_MB_JS_URL . 'wp-color-picker-alpha/wp-color-picker-alpha.min.js', array( 'wp-color-picker' ), WPP_MB_VER, true );
			$dependencies = array( 'wp-color-picker-alpha' );
		}
		wp_enqueue_script( 'rwmb-color', WPP_MB_JS_URL . 'color.js', $dependencies, WPP_MB_VER, true );
	}
