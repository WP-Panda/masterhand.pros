 ...
 */
class WPP_MB_Autocomplete_Field extends WPP_MB_Multiple_Values_Field {
	/**
	 * Enqueue scripts and styles.
	 */
	public static function admin_enqueue_scripts() {
		wp_enqueue_style( 'rwmb-autocomplete', WPP_MB_CSS_URL . 'autocomplete.css', '', WPP_MB_VER );
		wp_enqueue_script( 'rwmb-autocomplete', WPP_MB_JS_URL . 'autocomplete.js', array( 'jquery-ui-autocomplete' ), WPP_MB_VER, true );
	...
	public static function html( $meta, $field ) {
		...
		$field   = apply_filters( 'wpp_mb_autocomplete_field', $field, $meta );
		...
		if ( is_array( $field['options'] ) ) {
			...
			foreach ( $meta as $value ) {
				$label = apply_filters( 'wpp_mb_autocomplete_result_label', $value, $field );
				$html .= sprintf(
					$tpl,
					esc_html( $label ),
					esc_html__( 'Delete', 'meta-box' ),
