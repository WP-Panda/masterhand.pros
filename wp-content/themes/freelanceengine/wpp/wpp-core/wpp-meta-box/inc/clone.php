...
class WPP_MB_Clone {
	...
	public static function html( $meta, $field ) {
		...
		foreach ( $meta as $index => $sub_meta ) {
			...
			$input_html .= WPP_MB_Field::call( $sub_field, 'html', $sub_meta );
			$input_html  = WPP_MB_Field::filter( 'html', $input_html, $sub_field, $sub_meta );
	...
	public static function value( $new, $old, $post_id, $field ) {
		...
		if ( in_array( $field['type'], array( 'file', 'image' ), true ) ) {
			return WPP_MB_Field::call( $field, 'value', $new, '', $post_id );
		}

		foreach ( $new as $key => $value ) {
			$old_value   = isset( $old[ $key ] ) ? $old[ $key ] : null;
			$value       = WPP_MB_Field::call( $field, 'value', $value, $old_value, $post_id );
			$new[ $key ] = WPP_MB_Field::filter( 'sanitize', $value, $field );
		}

		// Remove empty clones.
		$new = array_filter( $new, 'WPP_MB_Helpers_Value::is_valid_for_field' );
	...
	public static function add_clone_button( $field ) {
		...
		$text = WPP_MB_Field::filter( 'add_clone_button_text', $field['add_button'], $field );
	...
	public static function remove_clone_button( $field ) {
		$text = WPP_MB_Field::filter( 'remove_clone_button_text', '<i class="dashicons dashicons-minus"></i>', $field );
