...
class WPP_MB_Core {
	...
	public function init() {
		load_plugin_textdomain( 'meta-box', false, plugin_basename( WPP_MB_DIR ) . '/languages/' );
	...
	public function register_meta_boxes() {
		$configs    = apply_filters( 'wpp_mb_meta_boxes', [] );
		$meta_boxes = wpp_mb_get_registry( 'meta_box' );

		foreach ( $configs as $config ) {
			if ( empty( $config ) ) {
				continue;
			}
			$meta_box = wpp_mb_get_meta_box( $config );
