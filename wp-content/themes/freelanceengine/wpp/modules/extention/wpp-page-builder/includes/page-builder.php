<?php
	...
	function fx_pbbase_save_post( $post_id, $post ) {
		...
		if ( 'wpp_rb' !== $post->post_type || ! current_user_can( $post_type->cap->edit_post, $post_id ) ) {
			return $post_id;
		}
		...
		} /* New data submitted, but it's different data than previously stored data, update it */ elseif ( $submitted_data && ( $submitted_data != $saved_data ) ) {
			update_post_meta( $post_id, 'fxpb', $submitted_data );
		} /* New data submitted is empty, but there's old data available, delete it. */ elseif ( empty( $submitted_data ) && $saved_data ) {
		...
		/*} /* == Always delete page builder data if page template not selected == */ /*else {
			delete_post_meta( $post_id, 'fxpb' );
		}*/
	}
	...
	function fxpb_render_rows( $post ) {
		...
		if ( ! $row_datas ) {
			...
			return;
		} /* Data available, hide default notice */ else {
		...
		foreach ( $row_datas as $order => $row_data ) {
			...
			if ( 'col-1' == $row_data[ 'type' ] ) {
                ...
                <div class="fxpb-row fxpb-col-1">
                    ...
                    <div class="fxpb-row-fields">
                        <textarea class="fxpb-row-input" name="fxpb[<?php echo $order; ?>][content]" data-field="content" placeholder="Add HTML here..."><?php echo esc_textarea(
								$row_data[ 'content' ]
							); ?></textarea>
                ...
                <div class="fxpb-row fxpb-col-2">
                    ...
                    <div class="fxpb-row-fields">
                        <div class="fxpb-col-2-left">
                            <textarea class="fxpb-row-input" name="fxpb[<?php echo $order; ?>][content-1]" data-field="content-1"
                                      placeholder="1st column content here..."><?php echo esc_textarea( $row_data[ 'content-1' ] ); ?></textarea>
                        </div><!-- .fxpb-col-2-left -->
                        <div class="fxpb-col-2-right">
                            <textarea class="fxpb-row-input" name="fxpb[<?php echo $order; ?>][content-2]" data-field="content-2"
                                      placeholder="2nd column content here..."><?php echo esc_textarea( $row_data[ 'content-2' ] ); ?></textarea>
                        </div><!-- .fxpb-col-2-right -->
	...
	function fx_pbbase_admin_scripts( $hook_suffix ) {
		...
		/* In Page Edit Screen */
		/* In Page Edit Screen */
		if ( 'wpp_rb' == $post_type && in_array( $hook_suffix, array ( 'post.php', 'post-new.php' ) ) ) {
			
			/* Load Editor/Page Builder Toggle Script */
			wp_enqueue_script(
				'wpp-pb-admin-editor-toggle', WPP_BT_NEW_URI . '/wpp_bt_framework/wpp-page-builder/assets/admin-editor-toggle.js', array ( 'jquery' ), WPP_PB_VERSION
			);
			
			/* Enqueue CSS & JS For Page Builder */
			wp_enqueue_style( 'wpp-pb-admin', WPP_BT_NEW_URI . '/wpp_bt_framework/wpp-page-builder/assets/admin-page-builder.css', array (), WPP_PB_VERSION );
			wp_enqueue_script(
				'wpp-pb-admin', WPP_BT_NEW_URI . '/wpp_bt_framework/wpp-page-builder/assets/admin-page-builder.js', array ( 'jquery', 'jquery-ui-sortable' ), WPP_PB_VERSION, true
			);
		}
	}
