...
class AE_Private_Message_Actions extends AE_PostAction {
	...
	public function ae_private_message_sync( $ask_final_bid_data = false ) {
		...
		if ( $request['method'] == 'create' ) {
			...
			$parent = ! empty( $request['post_parent'] ) ? $request['post_parent'] : $ask_final_bid_data['project_id'];

			if ( ae_user_role( $user_ID ) == EMPLOYER ) {
				update_post_meta( $parent, 'freelancer_has_read', false );
			} else if ( ae_user_role( $user_ID ) == FREELANCER ) {
				update_post_meta( $parent, 'employer_has_read', false );
			}
		} else {
			// if read dialog
			if ( ae_user_role( $user_ID ) == EMPLOYER ) {
