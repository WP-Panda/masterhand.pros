...
if ( class_exists( 'wpp_Email', false ) ) {
	return;
}
 ...
 * Email Class
 *
 * wpp Email Class which is extended by specific email template classes to add emails to wpp
 *
 * @class       wpp_Email
 * @version     2.5.0
 * @package     wpp/Classes/Emails
 * @extends     wpp_Settings_API
 */
class wpp_Email extends wpp_Settings_API {
	...
	public function __construct() {
		...
		add_action( 'wpp_update_options_email_' . $this->id, array( $this, 'process_admin_options' ) );
	}
	...
	public function format_string( $string ) {
		...
		if ( has_filter( 'wpp_email_format_string_replace' ) || has_filter( 'wpp_email_format_string_find' ) ) {
			...
			$string = str_replace( apply_filters( 'wpp_email_format_string_find', $legacy_find, $this ), apply_filters( 'wpp_email_format_string_replace', $legacy_replace, $this ), $string );
		}
		 ...
		 * @since 3.2.0
		 */
		return apply_filters( 'wpp_email_format_string', str_replace( $find, $replace, $string ), $this );
	}
	...
	public function setup_locale() {
		if ( $this->is_customer_email() && apply_filters( 'wpp_email_setup_locale', true ) ) {
			wpp_switch_to_site_locale();
		}
	}
	...
	public function restore_locale() {
		if ( $this->is_customer_email() && apply_filters( 'wpp_email_restore_locale', true ) ) {
			wpp_restore_locale();
		}
	}
	...
	public function get_additional_content() {
		...
		return apply_filters( 'wpp_email_additional_content_' . $this->id, $this->format_string( $content ), $this->object, $this );
	}
	...
	public function get_subject() {
		return apply_filters( 'wpp_email_subject_' . $this->id, $this->format_string( $this->get_option( 'subject', $this->get_default_subject() ) ), $this->object, $this );
	}
	...
	public function get_heading() {
		return apply_filters( 'wpp_email_heading_' . $this->id, $this->format_string( $this->get_option( 'heading', $this->get_default_heading() ) ), $this->object, $this );
	}
	...
	public function get_recipient() {
		$recipient  = apply_filters( 'wpp_email_recipient_' . $this->id, $this->recipient, $this->object, $this );
	...
	public function get_headers() {
		...
		return apply_filters( 'wpp_email_headers', $header, $this->id, $this->object, $this );
	}
	...
	public function get_attachments() {
		return apply_filters( 'wpp_email_attachments', array(), $this->id, $this->object, $this );
	}
	...
	public function get_content_type( $default_content_type = '' ) {
		...
		return apply_filters( 'wpp_email_content_type', $content_type, $this, $default_content_type );
	}
	...
	public function get_title() {
		return apply_filters( 'wpp_email_title', $this->title, $this );
	}
	...
	public function get_description() {
		return apply_filters( 'wpp_email_description', $this->description, $this );
	}
	...
	public function get_option( $key, $empty_value = null ) {
		...
		return apply_filters( 'wpp_email_get_option', $value, $this, $value, $key, $empty_value );
	}
	...
	public function is_enabled() {
		return apply_filters( 'wpp_email_enabled_' . $this->id, 'yes' === $this->enabled, $this->object, $this );
	}
	...
	public function style_inline( $content ) {
		if ( in_array( $this->get_content_type(), array( 'text/html', 'multipart/alternative' ), true ) ) {
			ob_start();
			wpp_get_template( 'emails/email-styles.php' );
			$css = apply_filters( 'wpp_email_styles', ob_get_clean(), $this );
			...
			if ( $this->supports_emogrifier() && class_exists( $emogrifier_class ) ) {
				try {
					...
					do_action( 'wpp_emogrifier', $emogrifier, $this );
	...
	public function get_from_name( $from_name = '' ) {
		$from_name = apply_filters( 'wpp_email_from_name', get_option( 'wpp_email_from_name' ), $this, $from_name );
	...
	public function get_from_address( $from_email = '' ) {
		$from_email = apply_filters( 'wpp_email_from_address', get_option( 'wpp_email_from_address' ), $this, $from_email );
	...
	public function send( $to, $subject, $message, $headers, $attachments ) {
		...
		$message              = apply_filters( 'wpp_mail_content', $this->style_inline( $message ) );
		$mail_callback        = apply_filters( 'wpp_mail_callback', 'wp_mail', $this );
		$mail_callback_params = apply_filters( 'wpp_mail_callback_params', array( $to, $subject, $message, $headers, $attachments ), $this );
	...
	public function init_form_fields() {
		...
		$placeholder_text  = sprintf( __( 'Available placeholders: %s', 'wpp' ), '<code>' . esc_html( implode( '</code>, <code>', array_keys( $this->placeholders ) ) ) . '</code>' );
		$this->form_fields = array(
			'enabled'            => array(
				'title'   => __( 'Enable/Disable', 'wpp' ),
				'type'    => 'checkbox',
				'label'   => __( 'Enable this email notification', 'wpp' ),
				'default' => 'yes',
			),
			'subject'            => array(
				'title'       => __( 'Subject', 'wpp' ),
				'type'        => 'text',
				'desc_tip'    => true,
			...
			'additional_content' => array(
				'title'       => __( 'Additional content', 'wpp' ),
				'description' => __( 'Text to appear below the main email content.', 'wpp' ) . ' ' . $placeholder_text,
				'css'         => 'width:400px; height: 75px;',
				'placeholder' => __( 'N/A', 'wpp' ),
				'type'        => 'textarea',
				'default'     => $this->get_default_additional_content(),
				'desc_tip'    => true,
			),
			'email_type'         => array(
				'title'       => __( 'Email type', 'wpp' ),
				'type'        => 'select',
				'description' => __( 'Choose which format of email to send.', 'wpp' ),
				'default'     => 'html',
	...
	public function get_email_type_options() {
		$types = array( 'plain' => __( 'Plain text', 'wpp' ) );

		if ( class_exists( 'DOMDocument' ) ) {
			$types['html']      = __( 'HTML', 'wpp' );
			$types['multipart'] = __( 'Multipart', 'wpp' );
		}
	...
	protected function save_template( $template_code, $template_path ) {
		if ( current_user_can( 'edit_themes' ) && ! empty( $template_code ) && ! empty( $template_path ) ) {
			...
			if ( ! $saved ) {
				$redirect = add_query_arg( 'wpp_error', rawurlencode( __( 'Could not write to template file.', 'wpp' ) ) );
	...
	public function get_theme_template_file( $template ) {
		return get_stylesheet_directory() . '/' . apply_filters( 'wpp_template_directory', 'wpp', $template ) . '/' . $template;
	}
	...
	protected function move_template_action( $template_type ) {
		...
		if ( ! empty( $template ) ) {
			...
			if ( wp_mkdir_p( dirname( $theme_file ) ) && ! file_exists( $theme_file ) ) {
				...
				$template_file = apply_filters( 'wpp_locate_core_template', $core_file, $template, $this->template_base, $this->id );
				 ...
				 */
				do_action( 'wpp_copy_email_template', $template_type, $this );

				?>
				<div class="updated">
					<p><?php echo esc_html__( 'Template file copied to theme.', 'wpp' ); ?></p>
				</div>
				<?php
			}
		}
		...
		if ( $template ) {
			if ( ! empty( $template ) ) {
				...
				if ( file_exists( $theme_file ) ) {
					 ...
					 * @param string $email The email object
					 */
					do_action( 'wpp_delete_email_template', $template_type, $this );
					?>
					<div class="updated">
						<p><?php echo esc_html__( 'Template file deleted from theme.', 'wpp' ); ?></p>
					</div>
					<?php
				}
			}
	...
	protected function admin_actions() {
		...
		if (
			...
			if ( empty( $_GET['_wpp_email_nonce'] ) || ! wp_verify_nonce( wpp_clean( wp_unslash( $_GET['_wpp_email_nonce'] ) ), 'wpp_email_template_nonce' ) ) {
				wp_die( esc_html__( 'Action failed. Please refresh the page and retry.', 'wpp' ) );
			}

			if ( ! current_user_can( 'edit_themes' ) ) {
				wp_die( esc_html__( 'You don&#8217;t have permission to do this.', 'wpp' ) );
			}

			if ( ! empty( $_GET['move_template'] ) ) {
				$this->move_template_action( wpp_clean( wp_unslash( $_GET['move_template'] ) ) );
			}

			if ( ! empty( $_GET['delete_template'] ) ) {
				$this->delete_template_action( wpp_clean( wp_unslash( $_GET['delete_template'] ) ) );
			}
		}
	}
	...
	public function admin_options() {
		...
		$this->admin_actions();
		?>
		<h2><?php echo esc_html( $this->get_title() ); ?> <?php wpp_back_link( __( 'Return to emails', 'wpp' ), admin_url( 'admin.php?page=wc-settings&tab=email' ) ); ?></h2>
		 ...
		 * @param string $email The email object
		 */
		do_action( 'wpp_email_settings_before', $this );
		?>
		 ...
		 * @param string $email The email object
		 */
		do_action( 'wpp_email_settings_after', $this );
		?>
		...
		if ( current_user_can( 'edit_themes' ) && ( ! empty( $this->template_html ) || ! empty( $this->template_plain ) ) ) {
			...
			<div id="template">
				<?php
				$templates = array(
					'template_html'  => __( 'HTML template', 'wpp' ),
					'template_plain' => __( 'Plain text template', 'wpp' ),
				);
				...
				foreach ( $templates as $template_type => $title ) :
					...
					$core_file     = $this->template_base . $template;
					$template_file = apply_filters( 'wpp_locate_core_template', $core_file, $template, $this->template_base, $this->id );
					$template_dir  = apply_filters( 'wpp_template_directory', 'wpp', $template );
					?>
					<div class="template <?php echo esc_attr( $template_type ); ?>">
						...
						<?php if ( file_exists( $local_file ) ) : ?>
							<p>
								...
								<?php if ( is_writable( $local_file ) ) : // phpcs:ignore WordPress.VIP.FileSystemWritesDisallow.file_ops_is_writable ?>
									<a href="<?php echo esc_url( wp_nonce_url( remove_query_arg( array( 'move_template', 'saved' ), add_query_arg( 'delete_template', $template_type ) ), 'wpp_email_template_nonce', '_wpp_email_nonce' ) ); ?>" class="delete_template button">
										<?php esc_html_e( 'Delete template file', 'wpp' ); ?>
									</a>
								<?php endif; ?>

								<?php
								/* translators: %s: Path to template file */
								printf( esc_html__( 'This template has been overridden by your theme and can be found in: %s.', 'wpp' ), '<code>' . esc_html( trailingslashit( basename( get_stylesheet_directory() ) ) . $template_dir . '/' . $template ) . '</code>' );
								?>
							</p>
						...
						<?php elseif ( file_exists( $template_file ) ) : ?>
							<p>
								...
								if ( is_writable( $target_dir ) ) : // phpcs:ignore WordPress.VIP.FileSystemWritesDisallow.file_ops_is_writable
									?>
									<a href="<?php echo esc_url( wp_nonce_url( remove_query_arg( array( 'delete_template', 'saved' ), add_query_arg( 'move_template', $template_type ) ), 'wpp_email_template_nonce', '_wpp_email_nonce' ) ); ?>" class="button">
										<?php esc_html_e( 'Copy file to theme', 'wpp' ); ?>
									</a>
								<?php endif; ?>

								<?php
								/* translators: 1: Path to template file 2: Path to theme folder */
								printf( esc_html__( 'To override and edit this email template copy %1$s to your theme folder: %2$s.', 'wpp' ), '<code>' . esc_html( plugin_basename( $template_file ) ) . '</code>', '<code>' . esc_html( trailingslashit( basename( get_stylesheet_directory() ) ) . $template_dir . '/' . $template ) . '</code>' );
								?>
							</p>
						...
						<?php else : ?>
							<p><?php esc_html_e( 'File was not found.', 'wpp' ); ?></p>
						<?php endif; ?>
					</div>
				<?php endforeach; ?>
			</div>
			...
			wpp_enqueue_js(
				...
				var view = '" . esc_js( __( 'View template', 'wpp' ) ) . "';
				var hide = '" . esc_js( __( 'Hide template', 'wpp' ) ) . "';
				...
				jQuery( 'a.delete_template' ).click( function() {
					if ( window.confirm('" . esc_js( __( 'Are you sure you want to delete this template file?', 'wpp' ) ) . "') ) {
						return true;
					}
