...
//        $wpdb->delete("{$wpdb->prefix}pro_paid_users",['id' => $id],['%d']);
        ...
        if (ae_user_role($user_id) == FREELANCER) {
            $wpdb->update("{$wpdb->prefix}pro_paid_users",array('status_id' => PRO_BASIC_STATUS_FREELANCER),array('id' => $id));
        } else
            $wpdb->update("{$wpdb->prefix}pro_paid_users",array('status_id' => PRO_BASIC_STATUS_EMPLOYER),array('id' => $id));
    }
