...
foreach ( $profile_categories as $item ) {
	$query_args['tax_query'] = [
		[
			...
			'field'    => 'slug',
			'terms'    => [ $item->slug ] //excluding the term you dont want.
		]
	];
        ...
        <div class="profile-freelance-portfolio colaps">
            <div class="row">
                <div class="<?php echo $is_edit ? 'col-sm-6' : '' ?> col-xs-12">
                    <div data-toggle="collapse" data-target="#portfolio_cat-<?php echo $n ?>"
                         class="freelance-portfolio-title"><?php _e( 'Portfolio', ET_DOMAIN ) ?> <?php echo $item->name; ?>
                        <i
			...
			<?php if ( have_posts() and ! $is_edit ) { ?>
                <ol id="portfolio_cat-<?php echo $n ?>" class="collapse freelance-portfolio-list big-list row">
					<?php
					$postdata = [];
					while ( have_posts() ) {
						the_post();
						...
						$postdata[] = $convert;
						get_template_part( 'template/portfolio', 'item_num_list' );
					}
					?>
                </ol>
			<?php } ?>
