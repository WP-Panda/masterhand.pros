<?php
/**
 * Template Name: My special Offers
 */
	...
	<div class="fre-page-wrapper my-adverts">
        ...
        <div class="fre-page-section">
            <div class="container">
                ...
                <div class="btn-wrap">
    				<a class="fre-submit-btn btn-right" href="<?php echo et_get_page_link( "create-advert" ) ?>"><?php _e( 'Add new', ET_DOMAIN ); ?></a>
                </div>				
				<?php } else { ?>
                <div class="ac-activate btn-wrap">
                    <?php _e( 'Activate Account Pro for Create Ad', ET_DOMAIN ); ?>
            	    <a href="/pro" class="btn-submit btn-right fre-submit-btn go-to-pro-account"><? _e('Activate') ?></a></div>
				<? } ?>
					<?php
							...
							if ( $freelancer_current_project_query->have_posts() ) {
								while ( $freelancer_current_project_query->have_posts() ) {
									...
									<div class="adverts_list_wp">
										<div class="adverts_t <?php if ( $status == 'archive' ) { echo 'project-status-archive'; } ?>" data-title="<?=$post->post_title;?>">
										...
										<div class="adverts_txt <?php if ( $status == 'archive' ) {echo 'project-status-archive'; } ?>">
										...
										<div class="btn-wrap">
											<? if($status == 'publish') { ?>
												<a class="fre-submit-btn btn-left" href="<?=get_permalink();?>?post_edit"><?php _e('Edit'); ?></a>
												<a class="advert-action archive cancel-btn" data-id="<?=$post->ID;?>"><?=__('Archive', ET_DOMAIN);?></a>
												<?
											}
...
//											}
							...
							<?php } ?>
									<script type="data/json" id="current_project_post_data"><?php echo json_encode( $postdata ); ?></script>
							<?php } else {
