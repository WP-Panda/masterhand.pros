...
if ( ! $user_ID ) {
	...
	$comments    = get_comments( [ 'type' => 'comment', 'post_id' => $post->ID, 'status' => 'approve' ] );
} else {
	$total_count = get_comments( [
		'post_id' => $post->ID,
		'type'    => 'comment',
	...
	$comments    = get_comments( [ 'type' => 'comment', 'post_id' => $post->ID, 'status' => 'all' ] );
}
?>
    <div id="comments" class="project-comments comments-area et-comments-area">
		...
		<?php if ( ! in_array( $post->post_status, [ 'disputed', 'close', 'complete', 'archive' ] ) ): ?>
            <div class="btm-comment-form">
				<?php
				$loginURL = et_get_page_link( 'login', [ 'ae_redirect_url' => get_permalink( $post->ID ) ] );
				comment_form( [
					...
					'logged_in_as'         => '<p class="logged-in-as">' . sprintf( __( 'Logged in as %1$s. <a href="%2$s" title="Log out of this account">Log out?</a>', ET_DOMAIN ), $current_user->display_name, wp_logout_url( apply_filters( 'the_permalink', get_permalink() ) ) ) . '</p>',
				] );
				?>
            </div>
		<?php endif; ?>
