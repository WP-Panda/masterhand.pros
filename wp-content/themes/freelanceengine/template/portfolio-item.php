...
<li class="col-sm-4 col-md-3 col-lg-3 col-sx-12">
    <div class="freelance-portfolio-wrap" id="portfolio_item_<?php echo $current->ID ?>">
        <div class="freelance-portfolio"
             style="background:url(<?php echo $current->the_post_thumbnail_full; ?>) center no-repeat;">
		...
		<?php if ( $is_edit ) { ?>
            <div class="portfolio-action">
                <a href="javascript:void(0)" class=" fre-submit-btn btn-center edit-portfolio"
                   data-id="<?php echo $current->ID ?>"><?php _e( 'Edit', ET_DOMAIN ) ?></a>

                <a href="javascript:void(0)" class="fre-cancel-btn btn-center remove_portfolio"
                   data-portfolio_id="<?php echo $current->ID ?>"><?php _e( 'Remove', ET_DOMAIN ) ?></a>
            </div>
		<?php } ?>
    </div>
</li>