...
<div class="workspace-project-box <?php if ($post->post_status == 'complete') { echo 'new-height';}?>">
    <ul class="nav nav-tabs nav-tabs-workspace hidden-lg hidden-md" role="tablist">
        <li class="active"><a href="#workspace-conversation-wp" data-group="conversation" data-toggle="tab" role="tab"><span><?php _e( 'Conversation', ET_DOMAIN ); ?></span></a></li>
        <li class="next">
    ...
    <div class="row">
								...
								foreach ( $comments as $key => $message ) {
									...
									if ( ! $message->isFile ) {
										if ( $message->changed_milestone_id != '' ) {
											...
											?>
                            <li class="<?php echo $message->user_id == $user_ID ? '' : 'partner-message' ?>" id="comment-<?php echo $message->comment_ID; ?>">
        ...
        <div id="workspace-files-wp" class="col-lg-4 col-md-4 col-sm-12 col-xs-12 tab-pane fade">
            ...
            <div id="workspace-files">
                ...
                <div class="workspace-files workplace-project-details <?php if (ae_user_role() != FREELANCER) {echo 'employer';};?>">
                    <div class="content-require-project content-require-project-attachment active">
                        ...
                        <?php  $attachment_comments = get_comments( array(
							'post_id'    => $post->ID,
							'meta_query' => array(
								array(
									'key'     => 'fre_comment_file',
                        ...
                        <div class="workplace-title-arrow file-container" id="file-container" style="font-size: 0;">
                            <div id="apply_docs_container">
                                ...
                                <span class="project_id" data-project="<?php echo $post->ID ?>"></span>
                                <span class="author_id" data-author="<?php echo $user_ID ?>"></span>
                                <a href="#" class="workspace-add-files fre-submit-btn attack attach-file" id="apply_docs_browse_button"><i class="fa fa-plus"></i><span><?php _e( 'Add file', ET_DOMAIN ); ?></span></a>
                            </div>
                        </div>
                        <?php }
						...
						?>
                        <div class="workspace-files-wrap <?php if (ae_user_role() != FREELANCER) {echo 'employer';}?>">
                            <ul class="workspace-files-list <?php if (ae_user_role() != FREELANCER) {echo 'employer';}?>" id="workspace_files_list">
                            <?php
                            ...
                            <li class="attachment-<?php echo $value->ID; ?>">
                                <?php  if ($value->post_mime_type == 'image/png') { echo '<i class="mime_type png"></i>';}
                                      else if ($value->post_mime_type == 'image/jpg' || $value->post_mime_type == 'image/jpeg') { echo '<i class="mime_type jpg"></i>'; }
                                      else if ($value->post_mime_type == 'image/gif' ) { echo '<i class="mime_type gif"></i>'; }
                                      else if ($value->post_mime_type == 'application/pdf' ) { echo '<i class="mime_type pdf"></i>';}
                                      else if ($value->post_mime_type == 'application/msword' ) { echo '<i class="mime_type doc"></i>';}
                                      else if ($value->post_mime_type == 'application/excel' || $value->post_mime_type == 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet') { echo '<i class="mime_type xls"></i>';}
                                      else if ($value->post_mime_type == 'image/bmp' ) { echo '<i class="mime_type bmp"></i>';}
                                      else { echo '<i class="mime_type doc"></i>'; }?>
