        ...
        <div class="row">
            <div class="col-lg-9 col-md-9 col-sm-8 col-xs-12">
                ...
                <ul class="project-bid-info-list row">
                    ...
                    <!--new-->
                    <li class="col-lg-9 col-md-9 col-sm-12 col-xs-12"><span><?php _e('Location:', ET_DOMAIN); ?></span><span><?php echo $project->str_location; ?></span></li>
                    <!--new-->
                    <!--new2-->
            ...
            <div class="col-md-3 col-lg-3 col-sm-4 col-xs-12">
                ...
                <div class="project-detail-action">
                    ...
                    if (is_user_logged_in()) {
                        if ($project_status == 'publish') {
                            if ((fre_share_role() || $user_role == FREELANCER) && $user_ID != $project->post_author) {
                                ...
                                if ($has_bid) { ?>
                                    ...
                                    <a class="fre-action-btn fre-submit-btn btn-right bid-action" data-action="edit" data-bid-id="<?php echo $bidding_id?>">
                                        <?php echo __('Edit', ET_DOMAIN)?>
                                    </a>

                                    <a class="fre-cancel-btn btn-right bid-action" data-action="cancel" data-bid-id="<?php echo $bidding_id?>">
                            ...
                            if ((int)$project->post_author == $user_ID || $bid_accepted_author == $user_ID || current_user_can('manage_options')) {
                                echo '<a class="fre-submit-btn btn-right" href="' . add_query_arg(array('dispute' => 1), $project_link) . '">' . __('Dispute Page', ET_DOMAIN) . '</a>';
                            }
                        } else if ($project_status == 'close') {
                            $bid_accepted_author = get_post_field('post_author', $bid_accepted);
                            if ((int)$project->post_author == $user_ID || $bid_accepted_author == $user_ID) {
                                echo '<a class="fre-submit-btn btn-right" href="' . add_query_arg(array('workspace' => 1), $project_link) . '">' . __('Workspace', ET_DOMAIN) . '</a>';
                            ...
                            if ((fre_share_role() || $user_role == EMPLOYER) && $user_ID == $project->post_author) {
                                echo '<a class="fre-action-btn btn-right fre-submit-btn" href="' . et_get_page_link('edit-project', array('id' => $project->ID)) . '">' . __('Edit', ET_DOMAIN) . '</a>';
                            ...
                            if ((fre_share_role() || $user_role == EMPLOYER) && $user_ID == $project->post_author) {
                                echo '<a class="fre-action-btn fre-submit-btn btn-right" href="' . et_get_page_link('edit-project', array('id' => $project->ID)) . '">' . __('Edit', ET_DOMAIN) . '</a>';
                            }
                        } else if ($project_status == 'draft') {
                            if ((fre_share_role() || $user_role == EMPLOYER) && $user_ID == $project->post_author) {
                                echo '<a class="fre-action-btn fre-submit-btn btn-right" href="' . et_get_page_link('submit-project', array('id' => $project->ID)) . '">' . __('Edit', ET_DOMAIN) . '</a>';
                            ...
                            if ((fre_share_role() || $user_role == EMPLOYER) && $user_ID == $project->post_author) {
                                echo '<a class="fre-action-btn fre-submit-btn btn-right" href="' . et_get_page_link('submit-project', array('id' => $project->ID)) . '">' . __('Renew', ET_DOMAIN) . '</a>';
                                echo '<a class="fre-action-btn fre-cancel-btn btn-right project-action" data-action="delete" data-project-id="' . $project->ID . '">' . __('Delete', ET_DOMAIN) . '</a>';
                            } else if (current_user_can('manage_options')) {
                                echo '<a class="fre-action-btn fre-cancel-btn btn-right project-action" data-action="delete" data-project-id="' . $project->ID . '">' . __('Delete', ET_DOMAIN) . '</a>';
                            }
                        ...
                        if ($project_status == 'publish') {
                            echo '<a class="fre-submit-btn btn-right" href="' . et_get_page_link('login', array('ae_redirect_url' => $project->permalink)) . '">' . __('Bid', ET_DOMAIN) . '</a>';
                        }
                    }
                    ?>
                <? if (($author_id == $user_ID) && $user_status && ($project_status == 'publish' || $project_status == 'draft' || $project_status == 'pending' ) ) {?>
                    <a class="btn-right fre-cancel-btn" href="<?php echo bloginfo('url');?>/options-project/?id=<?php echo $project->ID;?>" target="_blank"><? _e("Add Options", ET_DOMAIN);?></a>
                    <?php } ?>
                </div>
            </div>
        </div>
