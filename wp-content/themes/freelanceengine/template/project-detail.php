                ...
                <div class="row">
                    <div class="col-lg-4 col-md-4 col-sm-4 col-xs-12">
                        <a href="<?php echo get_author_posts_url( $post->post_author ); ?>"
                           class="avatar-author-project-item">
                    ...
                    <div class="col-lg-2 col-md-2 col-sm-2 col-xs-12 hidden-sm">
                        <span class="time-post-project-item"><span
                                    class="visible-xs-inline"><?php _e( "POSTED DATE:", ET_DOMAIN ); ?></span><?php the_date(); ?></span>
                    </div>
                    <div class="col-lg-2 col-md-1 col-sm-2 col-xs-12">
                        <span class="budget-project-item"><span
                                    class="visible-xs-inline"><?php _e( "BUDGET:", ET_DOMAIN ); ?></span><?php echo $convert->budget; ?></span>
                    </div>
                    <div class="col-lg-2 col-md-2 text-right col-sm-4 col-xs-12 btn-fre-bid"
						...
						<?php
							if ( current_user_can( 'manage_options' ) && $project_status != 'close' ) {
                                ...
                                <a href="<?php echo et_get_page_link( 'login', [ 'ae_redirect_url' => $project->permalink ] ); ?>"
                                   class="btn-apply-project-item"><?php _e( 'Bid', ET_DOMAIN ); ?></a>
							<?php } else {
								$role = ae_user_role();
								switch ( $project_status ) {
									case 'publish':
										if ( ( fre_share_role() || $role == FREELANCER ) && $user_ID != $project->post_author ) {
											...
											if ( $has_bid ) {
												?>
                                                <a rel="<?php echo $project->ID; ?>" href="#"
                                                   id="<?php echo $has_bid; ?>"
                                                   title="<?php _e( 'Cancel this bidding', ET_DOMAIN ); ?>"
                                                   class="btn-del-project">
									...
									case 'close':
										if ( (int) $project->post_author == $user_ID ) { ?>
                                            <a title="<?php _e( 'Finish', ET_DOMAIN ); ?>" href="#"
                                               id="<?php the_ID(); ?>" class="btn-complete-project">
												<?php _e( 'Finish', ET_DOMAIN ); ?>
                                            </a>
											<?php if ( ae_get_option( 'use_escrow' ) ) { ?>
                                                <a title="<?php _e( 'Close', ET_DOMAIN ); ?>" href="#"
                                                   id="<?php the_ID(); ?>" class="btn-close-project">
											...
											if ( $bid_accepted_author == $user_ID && ae_get_option( 'use_escrow' ) ) {
												?>
                                                <a title="<?php _e( 'Discontinue', ET_DOMAIN ); ?>" href="#"
                                                   id="<?php the_ID(); ?>" class="btn-quit-project">
									...
									case 'complete' :
										...
										$comment = get_comments( [ 'status'  => 'approve',
										                           'type'    => 'fre_review',
										                           'post_id' => get_the_ID()
										] );
										...
										if ( $user_ID == $freelan_id && empty( $comment ) ) { ?>
                                            <a href="#" id="<?php the_ID(); ?>"
                                               class="btn-complete-project"><?php _e( 'Review & Rate', ET_DOMAIN ); ?></a>
											<?php
										}
										break;
								}
