...
if (!empty($users_pro_paid)) {
    ...
    foreach ($users_pro_paid as $pro_paid_id) {
        if (ae_user_role($pro_paid_id['user_id']) == FREELANCER) {
            $wpdb->update("{$wpdb->prefix}pro_paid_users", array('status_id' => PRO_BASIC_STATUS_FREELANCER), array('id' => $pro_paid_id['id']));
        } else
            $wpdb->update("{$wpdb->prefix}pro_paid_users", array('status_id' => PRO_BASIC_STATUS_EMPLOYER), array('id' => $pro_paid_id['id']));

        $message = ae_get_option('expired_pro_status_template');
        $message = str_replace('[status_data]', $pro_paid_id['status_name'], $message);

        $res = $mailing->wp_mail($pro_paid_id['user_email'], $subject, $message, array('user_id' => $pro_paid_id['user_id']));
