...
function set_function_for_add_pro_status($query_my)
{
    ...
    if(count($query_my) == 1) {
        ...
        if ($query_my && $query_my['post_'] == PROJECT && ($role_template == 'freelance' || $role_template == 'gost')) {
...
function filter_join_project($join)
{
    ...
    } else $query_my = !empty($_REQUEST['query']) ? $_REQUEST['query'] : null;
...
function get_user_pro_status($userId)
{
    ...
    if (empty($result)) {
        if (ae_user_role($userId)) {
            if (ae_user_role($userId) == FREELANCER) $result = PRO_BASIC_STATUS_FREELANCER;
            else $result = PRO_BASIC_STATUS_EMPLOYER;
        } else $result = PRO_BASIC_STATUS_FREELANCER;
    }
