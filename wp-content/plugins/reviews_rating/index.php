...
function rwRating_template_include($template){
	if(strpos($_SERVER['REQUEST_URI'], '/check-payment') !== false){
        add_filter('wp_title', function (){return __('Check payment') . ' | ';}, 1);
        status_header(200);
...
function give_endorsements_template_include($template){
	if(strpos($_SERVER['REQUEST_URI'], '/give-endorsements') !== false){
        add_filter('wp_title', function (){return __('Give Endorsements') . ' | ';}, 1);
        status_header(200);
...
{
	add_menu_page(
		__('Ratings Reviews', 'review_rating'),
		__('Ratings Reviews', 'review_rating'),
		'administrator',
		'reviews_rating',
		'reviews_rating_module',
		plugins_url('/img/review_rating_25.png', __FILE__)
	);
...
function employerReplyAction()
{
    ...
    */
    ...
    if ($id !== false) {
        ...
        // through notify
        do_action( 'reply_added', array('project' => $project_id, 'freelancer' => $freelancer_id ));
...
function freelancerReplyAction()
{
	...
	if($review->isAccess($postData)) {
		...
		if ($id !== false) {
            ...
            do_action( 'reply_added_emp', array('project' => $project_id, 'employer' => $employer_id ));
...
//				ReviewsRating\Base::outputJSON([$trzDetail]);
		...
		if($orderId) {
			...
			$order_data = [
					...
					'cancelUrl' => $homeUrl . '?' . http_build_query(['payment' => 'cancel', 'returnUrl' => $returnUrl]),
