 ...
 */
class PMUE_Model_Record extends PMUE_Model {
	/**
	 * Initialize model
	 ...
	 * @return PMUE_Model_List
	 */
	public function getRelated($model, $keyAssoc = NULL) {
		$related = new $model();
		if ( ! empty($this->id)) {
			if (is_null($keyAssoc)) {
				$defaultPrefix = strtolower(preg_replace('%^' . strtoupper(PMUE_Plugin::PREFIX) . '|_Record$%', '', get_class($this)));
				$keyAssoc = array();
		...
		return $related instanceof PMUE_Model_List ? $related->convertRecords() : $related;
	}
