<?php // User Submitted Posts - Template Tags
	...
	*/
	function usp_is_public_submission( $post_ID = false ) {
	...
	*/
	function usp_get_post_images( $post_ID = false ) {
		...
		if ( usp_is_public_submission( $post_ID ) ) {

			return get_post_meta( $post_ID, 'user_submit_image' );
	...
	*/
	if ( ! function_exists( 'usp_get_images' ) ) :
		...
		function usp_get_images( $size = false, $before = false, $after = false, $number = false, $postId = false ) {
			...
			if ( false === $postId || ! is_numeric( $postId ) ) {
				$postId = $post->ID;
			}
			if ( false === $number || ! is_numeric( $number ) ) {
				$number = apply_filters( 'usp_image_attachments', 100 );
			}
			if ( false === $size ) {
				$size = 'thumbnail';
			}
