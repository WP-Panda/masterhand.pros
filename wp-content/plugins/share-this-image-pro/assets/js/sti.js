	...
	$.fn.sti = function( options ) {
		...
		var methods = {
			...
			shareData: function(el, network) {
				...
				data.media  = e.data('media') ? e.data('media') :  $('.banner_social').attr('src');
				...
				data.link     =  e.data('url') ? e.data('url') : $('#author_url').val() !== '' && $('#author_url').val() !== undefined ? $('#author_url').val() : data.local + data.hash;
		...
		if ( !opts.is_mobile ) {
			...
			if ( opts.always_show ) {
				...
				if ( ! $('#sti-box').length ) {
					$('.sharing').append('<div class="sti sti-share-box ">' + methods.shareButtons() + '</div>');
				}
            ...
            if ( network === 'link' ) {
            	...
            	//new
                // methods.emailBox(this, data, e); //old

				$('#modal_send_banner_to_emails').modal();

				$('.add-new-email').on('click', function(){
					var list_emails = $('#select_emails');
					var new_email = $('#add_new_email').val();

					if (new_email.length > 0 && (new_email.match(/.+?\@.+/g) || []).length !== 1) {
						alert('You have entered an invalid e-mail!');
					} else {
						if (new_email !== '' && !list_emails.find("option[value='" + new_email + "']").length) {
							// Create a DOM Option and pre-select by default
							var newOption = new Option(new_email, new_email, true, true);
							// Append it to the select
							list_emails.append(newOption).trigger('change');
		...
		$('document').ready(function () {
			$('.form-send-banner-to-emails').on('submit', saveEmails);
			...
			function parseJsonString(str) {
				return isJsonString(str) ? JSON.parse(str) : (typeof str === 'object' ? str : {});
			}
			...
			function saveEmails(event){
				var pData = {};
				pData['action'] = 'share_banner_to_email';
				pData['img_src'] = $('.img_show')[0].src;
				pData['emails'] = $('#select_emails').val();

				blockUi.block(event.currentTarget);
				$.post(ajax_var.url, pData, function(result){
					var data = parseJsonString(result)
					...
					if(data.status == 'success') {
						...
						$('#modal_send_banner_to_emails').modal('hide');
						$("#select_emails option").remove();
						blockUi.unblock();
					} else {
						showError(data.msg)
					}
				}).fail(failRequest).always(function(){blockUi.unblock();})

				return false;
			}

			if ($('form').hasClass('form-send-banner-to-emails')) {
