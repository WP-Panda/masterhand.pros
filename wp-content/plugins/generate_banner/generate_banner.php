...
function page_banner_template_include($template){
    if(strpos($_SERVER['REQUEST_URI'], '/referrals') !== false){
        add_filter('wp_title', function (){return __('Referrals') . ' | ';}, 1);
        status_header(200);
...
{
    if (!empty($_POST) && !empty($_POST['action']) && $_POST['action'] == 'save_banner' && empty($_POST['img_str'])) exit;
