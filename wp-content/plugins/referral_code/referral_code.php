 ...
 *
 * @wordpress-plugin
 * Plugin Name:       Referral code
 * Description:       Description referral code.
 * Version:           1.0.0
 * Author:            web13
 */

use ReviewsRating\Log;
...
function activate_referral_code()
{
    require_once 'includes/class-referral-code-activator.php';
    Referral_Code_Activator::activate();
}

function deactivate_referral_code()
{
    require_once 'includes/class-referral-code-deactivator.php';
    Referral_Code_Deactivator::deactivate();
}

register_activation_hook(__FILE__, 'activate_referral_code');
register_deactivation_hook(__FILE__, 'deactivate_referral_code');
...
function generate_referral_code($user_id)
{
    ...
    if (!empty($user_id)) {
        ...
        if (!empty($_REQUEST['code'])) {
            ...
            $referral_code = !empty($_REQUEST['referral_code']) ? (int)$_REQUEST['referral_code']
                : (!empty($_REQUEST['referral-code']) ? (int)$_REQUEST['referral-code'] : null);
        }

//        $referral_code = !empty($_REQUEST['code']) ? $_REQUEST['code'] : null;
//        $referral_code = !empty($_POST['referral_code']) ? (int)$_POST['referral_code'] : null;
        ...
        $wpdb->insert(
            TABLE_REFERRAL,
            array('user_id' => $user_id, 'referral_code' => $code, 'user_id_referral' => $referral_user_id),
            array('%d', '%d', '%d')
        );
    }
}
...
function outputJSON($data = array(), $status = false)
{
    $response = [];
...
{
    wp_enqueue_style('myPluginStylesheet', plugins_url('css/referral_code_style.css', __FILE__));
...
function set_referral_code_by_old_user($user_id)
{
    ...
    if (!empty($user_id)) {
        ...
        $wpdb->insert(
            TABLE_REFERRAL,
            array('user_id' => $user_id, 'referral_code' => $code, 'user_id_referral' => 0),
            array('%d', '%d', '%d')
        );
    }
}
